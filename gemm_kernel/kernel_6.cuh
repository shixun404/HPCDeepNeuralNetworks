__global__  __launch_bounds__(64) void sgemm_6(int N, float *A, float *B, float *C, float alpha, float beta){
    ...
    for(int k = 0; k < gridDim.x; ++k){
        ...
        *(float4*)(shared_B + tidx + (1 + tidy) * blockDim.y * 4) = *(float4*)(B + i1 + (jj1 + 1) * N);
        *(float4*)(shared_B + tidx + (2 + tidy) * blockDim.y* 4) = *(float4*)(B + i1 + (jj1 + 2) * N);
        *(float4*)(shared_B + tidx + (3 + tidy) * blockDim.y* 4) = *(float4*)(B + i1 + (jj1 + 3) * N);
        __syncthreads();
