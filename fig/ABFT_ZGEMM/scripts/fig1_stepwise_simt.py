...
tensor_core_kernel = th.as_tensor([9100, 15000, 17200, 17800, 18200, 18700, 18900, 19000 ])
...
tensor_core_kernel_name = ['SIMT', 'wmma',  'mma.sync', 'Shared Memory\nBank Conflict',  'Triple Buffer/\n3-Stage', 'Async Copy', 'CUTLASS', 'cuBLAS']
error = th.rand(11) * 0.03

br1 = np.arange(len(simt_kernel_name)) + barWidth
...
# Make the plot
ax.bar(br1, simt_kernel / 1000, color = color[2], width = barWidth,
        edgecolor ='k', label='ZGEMM (Ours)', zorder=3)
plt.plot([0, 7.3], [9.7, 9.7], linestyle='--', color='k', linewidth='3', label='FP64 Roofline')
...
ax.set_xticks(br1) 
ax.set_xticklabels([f'{simt_kernel_name[i]}' for i in range(len(simt_kernel))], rotation = 30)
plt.title("ZGEMM Stepwise Optimization (SIMT)", fontdict=dict(weight='bold'))
ax.set_xlim([0, 7.3])
ax.set_yticks([0, 2, 4, 8, 10])

ax.legend(loc="lower right", framealpha=0.7)
fig.savefig("../figures/fig1_stepwise_SIMT.pdf",bbox_inches='tight')
