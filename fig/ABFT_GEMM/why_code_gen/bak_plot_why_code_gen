...
ax1 = fig.add_axes(rect, axes_locator=div.new_locator(nx=0, ny=1),)
ax0 = fig.add_axes(rect, axes_locator=div.new_locator(nx=1, ny=0))
axLin = fig.add_axes(rect, axes_locator=div.new_locator(nx=0, ny=0),  )
ax = fig.add_axes(rect, axes_locator=div.new_locator(nx=1, ny=1),  )
...
performance_scale = 1
l = N.shape[0]
ax.plot(N, cublas[:l] / performance_scale,  label="cublas SGEMM", marker='o', markersize=ms, color = color[0], clip_on=False)
# ax.plot(N, abft_baseline[:l] / performance_scale, label="abft baseline", marker='^',markersize=ms,  color = color[3], clip_on=False, zorder=0)
ax.plot(N, kernel_sgemm[:l] / performance_scale,  label="SGEMM (Ours)", marker='P',markersize=ms, color = color[1], clip_on=False)
ax.plot(N, abft_kernel[:l] / performance_scale,  label="ABFT SGEMM (Ours)", marker='s',markersize=ms,  color = color[2], clip_on=False)

# ax0.set_xlabel("Matrix Sizes M=N=K")
...
axLin.set_ylim(1, 1024)
ax1.set_xlim(16, 512)
ax1.set_ylim(1000, 5300)
ax0.set_xlim(512, 10500)
ax0.set_ylim(1, 1024)
ax.set_ylim(1000, 5300)
ax.set_xlim(512, 10500)
ax.tick_params(axis='x', tick1On=False)
ax.tick_params(axis='y', tick1On=False)
ax.set_xticklabels(labels=[],alpha=1)
ax.set_yticklabels(labels=[],alpha=1)
ax1.tick_params(axis='x', tick1On=False)
ax0.tick_params(axis='y', tick1On=False)
yticks = [1, 4,16, 64, 256, 1024]
ax0.set_yticks(yticks)
ax1.set_xticklabels(labels=[],alpha=1)

ax0.set_yticklabels(labels=[],alpha=1)
...
l = N.shape[0]
axLin.plot(N, cublas[:l] / performance_scale,   marker='o', markersize=ms, color = color[0], clip_on=False)
# axLin.plot(N, abft_baseline[:l] / performance_scale,  marker='^',markersize=ms,  color = color[3], clip_on=False)
axLin.plot(N, kernel[:l] / performance_scale, '--' ,  marker='P',markersize=ms, color = color[1], clip_on=False)
axLin.plot(N, kernel_[:l] / performance_scale,  marker='P',markersize=ms, color = color[1], clip_on=False)
axLin.plot(N, abft[:l] / performance_scale, '--',  marker='s',markersize=ms,  color = color[2], clip_on=False)
axLin.plot(N, abft_[:l] / performance_scale,   marker='s',markersize=ms,  color = color[2], clip_on=False)
...
ax1.set_xticklabels([f'{s}' for s in xticks])
...
ax0.grid(True)
ax1.grid(True)
ax.legend(bbox_to_anchor=(1, -0.3), prop={'size': 20})
xticks = [1024, 2048, 4096, 8192]

yticks = [2000, 3000, 4000, 5000]
ax1.set_yticks(yticks)
ax1.set_yticklabels([f'{s}' for s in yticks])

ax0.set_xticks(xticks)
ax0.set_xticklabels([f'{s}' for s in xticks])
