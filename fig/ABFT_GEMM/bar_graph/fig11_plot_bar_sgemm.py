...
gemm = th.as_tensor([   75.55,  117.18,  175.94,  210.96, 404.86,  569.16,  557.98,  689.67, 982.96, 1077.14, 1226.84, 1247.60,
                      1023.60, 2687.45, 3651.31, 2736.64, 4944.31, 4885.88, 4901.90, 4861.72, ])
...
gemm_huge = th.as_tensor([15.33,   23.58,   34.28,   46.11, 93.06,  135.40,  184.68,  242.15,  531.38,  626.43,  725.27,  831.86, 400.56,  948.14, 1435.13, 2001.96, 4944.31, 4885.88, 4901.90, 4861.72,])
...
plt.yticks([0, 0.5, 1, 1.5, 2], ['0', '0.5', '1.0', '1.5', '2.0'], fontsize = 40,)
...
for i in range(5):
    x = 1.9 + 4 * i
    plt.text(x, 2.00, kernel_name[i], ha = 'center', fontsize=40) 
    plt.text(x, -0.65, kernel_size[i], ha = 'center', fontsize=40) 
plt.text( 1.9 + 8, -0.83, 'Matrix Sizes', ha = 'center', fontsize=40) 
# plt.title("Performance of generated kernels", fontdict=dict(weight='bold'))
plt.grid(linestyle='--', linewidth=0.7, zorder=0)
plt.legend(loc="lower right", framealpha=0.7, fontsize=32,labelspacing = 0)
plt.savefig("sgemm_bar_huge.pdf",bbox_inches='tight')
