...
ax[0].plot(N, abft_kernel_correct / 1000,  label="thread-level (k % 256 == 0), without correction", color = 'g')
...
ax[1].plot(N, 100 * (kernel_13 - abft_kernel_correct) / kernel_13,  label="thread-level (k % 256 == 0), without correction", color = 'g')
...
ax[1].set_title("(GEMM - FT_GEMM) / GEMM x 100%")
ax[1].grid()
ax[1].legend(loc="lower right", prop={'size': 10})

fig.savefig(f"abft_sgemm_thread_reg=34.png", dpi=300, bbox_inches='tight')