...
cublas = th.as_tensor([63.97,   97.73,  120.84,  168.33, 215.98,  305.02,  338.76,  410.21,1028.06,  896.00, 1064.46, 1275.35,514.29, 1465.11, 1747.82, 2005.86, 4078.11, 4099.19, 4427.79, 4377.52])
...
plt.bar(br3, gemm / cublas, color =color[2], width = barWidth,
        edgecolor ='k', label ='Our fused FT: off',zorder=3)
plt.bar(br1, bs / cublas, color = color[0], width = barWidth,
        edgecolor ='k', label ='Ding et al. 2011: FT on', zorder=3)
plt.bar(br4, abft / cublas, color =color[3], width = barWidth,
        edgecolor ='k', label ='Our fused FT: on',zorder=3)
for i in range(4):
    x = 3.8 + i * 4
...
for i in range(5):
    x = 1.9 + 4 * i
    plt.text(x, 2.00, kernel_name[i], ha = 'center') 
