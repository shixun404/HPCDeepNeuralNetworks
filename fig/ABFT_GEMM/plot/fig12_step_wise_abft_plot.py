import matplotlib.pyplot as plt
from mpl_toolkits.axes_grid1 import make_axes_locatable
import torch as th
import seaborn as sns
import matplotlib
matplotlib.rcParams['pdf.fonttype'] = 42
matplotlib.rcParams['ps.fonttype'] = 42
color = sns.color_palette(n_colors=4)
color1 = sns.color_palette("rocket", n_colors=6)
print(color[3])

ms = 8
N = th.as_tensor([i for i in range(256, 10240+256, 256)])

plt.rc('font', size=40, weight='bold')
plt.rcParams['lines.linewidth'] = 3
plt.rcParams["font.family"] = "Times New Roman"
fig, ax = plt.subplots(nrows=1, ncols=2, figsize=(16, 7), )
plt.tight_layout()
fig.subplots_adjust(hspace=0.1, wspace = 0.1)
def print_average(tensor, name):
    print(f"############## {name} #################")
    avg = (tensor.sum() - tensor.max() - tensor.min()) / tensor.shape[0]
    print(name, avg)
    print(tensor.sort())



cublas = th.as_tensor([  666.99, 2712.65, 3713.77, 3980.43, 4483.73, 4386.59, 4540.00, 4506.04, 4514.94, 4499.14, 4584.03, 4125.07, 4157.81, 4477.70, 4266.53, 4467.25, 4261.45, 4247.67, 4273.68, 3969.71, 4323.71, 4292.29, 4304.44, 3929.68, 4259.45, 4216.95, 4264.67, 4163.82, 4168.30, 4192.88, 4201.44, 4072.17, 4099.62, 4014.48, 4058.69, 4151.78, 4146.84, 3976.91, 3888.91, 3945.27,])
kernel_sgemm_huge = th.as_tensor([  463.93, 2063.97, 4575.27, 4406.37, 4747.41, 5101.23, 5073.51, 4772.37, 4669.83, 4863.54, 4696.61, 4827.34, 4597.03, 4767.20, 4652.09, 4761.49, 4699.69, 4701.10, 4644.34, 4685.55, 4640.78, 4625.96, 4624.65, 4600.52, 4586.89, 4574.40, 4585.92, 4539.97, 4533.83, 4504.52, 4525.99, 4488.19, 4542.69, 4502.66, 4507.64, 4462.35, 4503.41, 4445.23, 4422.88, 4428.44,])
abft_baseline = th.as_tensor([  206.78,  798.51, 1411.14, 1791.73, 2245.11, 2377.05, 2651.54, 3100.24, 3143.26, 3289.58, 3317.46, 3221.68, 3290.23, 3274.90, 3447.92, 3236.21, 3192.38, 3174.55, 3111.09, 3108.13, 3109.62, 3104.56, 3096.40, 3576.08, 3101.83, 3560.77, 3556.21, 3013.72, 3523.36, 3514.41, 3496.64, 2902.01, 2945.49, 3479.86, 3466.30, 3433.25, 3418.85, 3419.97, 3416.55, 3415.93,])
abft_kernel_huge = th.as_tensor([  290.65, 1265.30, 2754.32, 2709.93, 2897.86, 3182.36, 3761.84, 4212.58, 4129.75, 4338.51, 4159.72, 4244.09, 4043.43, 4166.67, 4104.33, 4195.57, 4108.32, 4134.47, 4121.45, 4129.44, 4123.65, 4116.19, 4104.56, 4097.50, 4082.90, 4084.53, 4096.08, 4054.60, 4057.37, 4022.79, 4009.75, 3978.42, 3961.11, 3949.90, 3916.70, 3892.21, 3915.23, 3872.23, 3862.16, 3850.78,])
warp_abft_kernel = th.as_tensor([  375.30, 1674.17, 3339.22, 3172.84, 3381.19, 3694.67, 3975.60, 3752.53, 3743.09, 3942.31, 3751.52, 3658.97, 3758.64, 3733.91, 3747.24, 3733.88, 3713.23, 3720.98, 3708.72, 3709.51, 3700.47, 3702.88, 3678.20, 3679.33, 3665.82, 3662.61, 3656.21, 3642.68, 3625.41, 3610.82, 3603.82, 3570.12, 3588.65, 3550.00, 3567.52, 3551.57, 3538.54, 3527.42, 3518.44, 3502.33,])
thread_abft_kernel = th.as_tensor([  368.24, 1469.37, 3109.12, 2780.08, 2876.95, 3143.80, 3425.25, 3234.73, 3216.62, 3499.99, 3276.78, 3352.04, 3399.71, 3349.52, 3216.45, 3280.92, 3339.85, 3293.58, 3190.57, 3232.09, 3235.34, 3194.45, 3243.18, 3222.99, 3219.66, 3221.14, 3228.75, 3218.88, 3228.68, 3228.66, 3218.75, 3222.16, 3221.23, 3209.84, 3216.96, 3205.04, 3214.64, 3205.02, 3202.11, 3196.47,])
l = N.shape[0]
roofline_model = th.ones_like(N) * 5600
print("\n\n #########################   M=N=K  #########################################")
print_average((warp_abft_kernel - abft_kernel_huge) / warp_abft_kernel, "warp vs tb")
print_average((thread_abft_kernel - abft_kernel_huge) / thread_abft_kernel, "thread vs tb")
print_average((abft_baseline - abft_kernel_huge) / abft_baseline, "abft baseline vs abft")

ax[0].plot(N, abft_baseline[:l] / 1000, label="Ding et al. 2011", marker='^',markersize=ms,  color = color[0], clip_on=False)
ax[0].plot(N, thread_abft_kernel[:l] / 1000, linestyle='dotted', label="Thread-level", marker='o',markersize=ms*0.9,  color = color[3], clip_on=False)
ax[0].plot(N, warp_abft_kernel[:l] / 1000,  linestyle='dashed',label="Warp-level", marker='x',markersize=ms,  color = color[3], clip_on=False)
ax[0].plot(N, abft_kernel_huge[:l] / 1000,  label="Threadblock-level", marker='s',markersize=ms,  color = color[3], clip_on=False)


ax[0].set_xlabel("(a)Matrix Sizes M=N=K",fontdict=dict(weight='bold',size=40))
ax[0].set_ylabel("Perf. (TFLOPS)", fontdict=dict(weight='bold',size=40))
# ax[0].set_xscale("log", base=2)
ax[0].set_xlim(0, 10240)
xticks = [0, 2000, 4000, 6000, 8000, 10000]#[256, 512, 1024, 2048, 4096, 8192]
ax[0].set_xticks(xticks)
ax[0].set_xticklabels(['0','2k','4k','6k','8k','10k',])
yticks = [1, 2, 3, 4]
ax[0].set_yticks(yticks)
ylabels = ['1', '2', '3', '4']
ax[0].set_yticklabels(ylabels)
# ax[0].set_xticklabels([f'{s}' for s in xticks])

ax[0].grid()
ax[0].legend(loc="lower right", prop={'size': 32, },labelspacing=0,bbox_to_anchor=(1.02,-0.03))

cublas = th.as_tensor([ 2003.63, 3266.27, 3775.61, 3926.27, 4349.67, 4131.79, 4312.81, 4758.74, 4724.33, 4716.27, 4722.18, 4604.74, 4791.88, 4495.17, 4377.72, 4225.43, 3656.11, 4087.09, 4080.16, 4130.00, 4086.89, 3977.68, 4046.44, 3811.12, 4013.54, 3880.55, 3952.28, 3885.93, 3950.50, 3884.05, 3913.96, 3913.18, 3873.30, 3942.18, 3982.52, 3966.93, 3962.62, 3956.13, 3951.24, 3946.20,])
kernel_sgemm_small = th.as_tensor([ 1340.87, 1565.67, 1652.85, 1539.17, 1133.25, 1046.21, 1028.68, 1053.25, 1007.70,  832.57,  984.92,  978.16,  889.09,  950.32,  946.60,  982.25,  945.58,  942.52,  936.19,  924.90,  932.95,  924.42,  918.39,  913.65,  909.37,  908.58,  903.19,  913.68,  909.29,  898.50,  895.66,  901.45,  883.25,  874.94,  868.92,  862.80,  857.36,  847.24,  841.21,  832.52,])
kernel_sgemm_medium = th.as_tensor([ 2139.79, 3099.60, 3394.53, 3338.78, 3040.56, 2130.77, 2133.19, 2160.26, 2088.57, 2010.31, 1971.99, 1878.44, 1622.03, 1949.41, 1954.06, 1748.28, 1933.41, 1759.77, 1887.31, 1801.02, 1911.13, 1838.62, 1831.97, 1851.63, 1848.90, 1846.08, 1848.55, 1839.32, 1846.81, 1837.27, 1831.53, 1831.58, 1825.64, 1826.66, 1825.24, 1819.71, 1820.05, 1814.59, 1812.49, 1809.08,])
kernel_sgemm_large = th.as_tensor([ 1310.78, 2833.80, 5123.27, 5067.46, 5160.48, 4342.85, 3326.38, 3395.82, 3571.50, 3525.07, 3438.04, 3352.83, 3285.60, 3206.55, 2566.44, 3281.99, 3196.32, 3120.25, 2754.35, 3108.35, 2969.22, 2946.31, 3032.44, 2908.20, 2978.85, 2973.28, 2951.54, 2984.01, 2997.22, 2948.08, 2977.73, 2940.44, 2979.83, 2954.60, 2978.04, 2940.36, 2967.98, 2946.56, 2949.33, 2930.04,])
kernel_sgemm_tall = th.as_tensor([ 1616.89, 2072.04, 2390.76, 2439.43, 2465.82, 2135.00, 2146.89, 2121.44, 2062.11, 2077.34, 2043.84, 2078.54, 1826.34, 1989.58, 1982.44, 1955.29, 1955.35, 1945.44, 1925.86, 1937.47, 1944.72, 1917.89, 1932.37, 1932.54, 1931.39, 1930.01, 1935.02, 1932.60, 1934.16, 1935.52, 1938.64, 1920.13, 1940.51, 1929.31, 1934.24, 1923.71, 1936.43, 1929.51, 1930.07, 1924.21,])
kernel_sgemm_wide = th.as_tensor([ 1984.58, 3651.79, 4240.55, 4307.93, 4346.83, 4253.14, 4326.50, 2376.34, 2739.12, 2906.92, 2894.99, 2858.56, 2787.55, 2184.47, 2913.53, 2951.90, 2909.92, 2601.72, 2835.35, 2860.48, 2685.86, 2856.04, 2696.03, 2816.42, 2744.21, 2759.98, 2825.92, 2768.75, 2772.78, 2784.06, 2801.97, 2763.19, 2797.67, 2781.37, 2797.65, 2775.87, 2798.22, 2785.70, 2787.33, 2774.38,])
kernel_sgemm_huge = th.as_tensor([  633.90, 2526.14, 4820.47, 4938.98, 5061.99, 4731.10, 4697.57, 4290.22, 4375.57, 4604.89, 4299.50, 4414.94, 4457.79, 4431.34, 4406.64, 3975.74, 4246.32, 4335.05, 4295.09, 4237.60, 3966.06, 4231.67, 4228.30, 3994.67, 4247.46, 4086.62, 4210.91, 4125.78, 4199.66, 4114.97, 4221.52, 4116.84, 4173.52, 4161.24, 4174.74, 4132.15, 4169.86, 4150.14, 4152.34, 4129.23,])
abft_baseline = th.as_tensor([  227.46,  783.65, 1428.72, 1795.53, 2240.51, 2382.09, 2634.95, 3190.68, 3373.91, 3415.36, 3383.12, 3407.25, 3395.98, 3379.23, 3460.14, 3155.75, 3365.93, 3421.34, 3241.57, 3166.52, 3383.59, 3271.48, 3286.13, 3674.79, 3249.47, 3676.48, 3681.44, 3299.13, 3691.43, 3695.90, 3712.04, 3338.27, 3312.66, 3747.49, 3697.23, 3677.81, 3682.33, 3680.93, 3685.99, 3689.18,])
abft_kernel_small = th.as_tensor([  856.36,  962.26, 1015.07,  807.11,  690.19,  660.43,  629.85,  636.24,  574.11,  621.46,  582.54,  602.86,  606.26,  596.12,  599.58,  626.64,  596.90,  598.52,  598.94,  597.36,  599.36,  597.55,  597.63,  596.04,  596.20,  597.18,  599.48,  597.50,  598.77,  596.43,  597.25,  598.23,  597.44,  595.99,  597.11,  596.44,  597.83,  597.16,  596.62,  595.13,])
abft_kernel_medium = th.as_tensor([ 1326.22, 2011.62, 2159.45, 2180.75, 1676.85, 1487.32, 1515.12, 1527.15, 1436.24, 1385.06, 1227.54, 1406.65, 1398.84, 1269.13, 1381.86, 1355.13, 1356.90, 1309.18, 1336.89, 1339.21, 1331.38, 1324.20, 1325.18, 1318.09, 1322.23, 1327.54, 1327.23, 1325.39, 1328.43, 1322.31, 1326.21, 1323.14, 1325.74, 1322.13, 1325.08, 1321.92, 1326.08, 1323.08, 1322.85, 1321.05,])
abft_kernel_large = th.as_tensor([  874.36, 1675.36, 2203.80, 2477.29, 2858.95, 2247.39, 2325.81, 2296.00, 2247.36, 2221.19, 2202.09, 2219.94, 2008.12, 2191.73, 2189.59, 2217.31, 2126.22, 2187.74, 2136.19, 2176.97, 2147.03, 2170.94, 2147.29, 2141.03, 2166.36, 2158.35, 2168.52, 2157.75, 2164.26, 2156.51, 2167.16, 2155.91, 2165.16, 2160.64, 2167.62, 2151.14, 2170.57, 2157.18, 2159.31, 2153.36,])
abft_kernel_tall = th.as_tensor([ 1249.10, 2282.01, 2600.71, 2773.01, 2504.81, 2039.77, 2058.40, 2036.56, 2011.67, 1962.88, 2000.69, 1956.66, 1853.37, 1944.75, 1933.69, 1921.95, 1930.06, 1875.80, 1891.80, 1854.22, 1885.05, 1854.87, 1854.68, 1865.63, 1866.98, 1861.58, 1871.81, 1867.44, 1869.09, 1867.34, 1872.39, 1858.89, 1875.86, 1863.84, 1868.76, 1859.68, 1872.12, 1864.21, 1864.43, 1860.65,])
abft_kernel_wide = th.as_tensor([ 1159.26, 2196.39, 2636.75, 2723.57, 2428.59, 1967.53, 2022.13, 2061.83, 2049.64, 2028.12, 2034.60, 2041.18, 1885.25, 2032.14, 2043.67, 2039.74, 2037.99, 2018.33, 2009.64, 2020.46, 2019.00, 2007.95, 2022.37, 2016.47, 2012.39, 2014.29, 2023.03, 2019.74, 2023.81, 2017.97, 2026.35, 2015.70, 2026.07, 2021.25, 2023.21, 2013.91, 2024.47, 2020.62, 2022.22, 2015.33,])
abft_kernel_huge = th.as_tensor([  392.10, 1558.76, 3297.31, 3195.79, 3414.47, 3730.14, 4059.51, 3850.22, 3968.04, 4268.98, 4011.49, 4196.96, 4154.85, 4081.40, 4058.32, 3937.87, 3877.93, 4055.19, 4027.12, 4013.27, 3911.02, 4046.35, 3999.77, 3954.24, 4034.75, 3941.47, 4036.09, 3945.05, 4022.21, 3966.31, 3978.07, 3978.01, 3988.09, 3961.21, 3986.64, 3967.22, 3980.05, 3963.72, 3964.22, 3950.13,])
warp_abft_kernel = th.as_tensor([  357.32, 1434.10, 2698.55, 2513.65, 2639.64, 2873.39, 3404.06, 3622.13, 3619.69, 3716.76, 3557.70, 3583.29, 3565.97, 3555.44, 3593.22, 3414.50, 3554.18, 3543.29, 3533.33, 3470.33, 3517.92, 3513.14, 3515.77, 3515.56, 3508.87, 3506.67, 3521.33, 3523.15, 3517.65, 3502.39, 3521.30, 3494.67, 3500.33, 3495.42, 3501.47, 3485.13, 3499.05, 3481.15, 3479.62, 3457.23,])
thread_abft_kernel = th.as_tensor([  377.54, 1507.81, 3201.60, 2847.61, 2951.73, 3208.30, 3374.38, 3104.30, 3101.53, 3366.82, 3134.25, 3238.71, 3283.93, 3212.00, 2985.23, 3147.34, 3132.15, 3132.30, 3031.98, 3093.05, 3100.87, 3063.76, 3092.90, 3059.32, 3085.65, 3055.63, 3090.39, 3082.51, 3076.74, 3078.57, 3080.58, 3078.89, 3075.11, 3058.89, 3066.83, 3061.43, 3060.92, 3054.63, 3051.17, 3042.91,])
l = N.shape[0]
print(kernel_sgemm_small.shape)
roofline_model = th.ones_like(N) * 5600
# ax[1].plot(N, cublas[:l] / 1000,  label="cublas SGEMM", marker='o', markersize=ms, color = color[0], clip_on=False)
ax[1].plot(N, abft_baseline[:l] / 1000, label="Ding et al. 2011", marker='^',markersize=ms,  color = color[0], clip_on=False)
# ax[1].plot(N, kernel_sgemm_huge[:l] / 1000,  label="SGEMM (Ours)", marker='P',markersize=ms, color = color[1], clip_on=False)
ax[1].plot(N, thread_abft_kernel[:l] / 1000, linestyle='dotted', label="Thread-level", marker='o',markersize=ms*0.9,  color = color[3], clip_on=False)
ax[1].plot(N, warp_abft_kernel[:l] / 1000,  linestyle='dashed',label="Warp-level", marker='x',markersize=ms,  color = color[3], clip_on=False)
ax[1].plot(N, abft_kernel_huge[:l] / 1000,  label="Threadblock-level", marker='s',markersize=ms,  color = color[3], clip_on=False)

print("\n\n #########################   M=N, K=1024  #########################################")
print_average((warp_abft_kernel - abft_kernel_huge) / warp_abft_kernel, "warp vs tb")
print_average((thread_abft_kernel - abft_kernel_huge) / thread_abft_kernel, "thread vs tb")
print_average((abft_baseline - abft_kernel_huge) / abft_baseline, "abft baseline vs abft")
ax[1].set_xlabel("(b)Matrix Sizes M=N, K=1024", fontdict=dict(weight='bold',size=40))
# ax[1].set_ylabel("Performance (TFLOPS)", fontdict=dict(weight='bold', size=40))
# ax[1].set_xscale("log", base=2)
ax[1].set_xlim(0, 10240)
xticks = [0, 2000, 4000, 6000, 8000, 10000]#[256, 512, 1024, 2048, 4096, 8192]
ax[1].set_xticks(xticks)
ax[1].set_xticklabels(['0','2k','4k','6k','8k','10k',])
# ax[1].set_xticklabels([f'{s}' for s in xticks])
yticks = [1, 2, 3, 4]
ax[1].set_yticks(yticks)
ylabels = ['1', '2', '3', '4']
ax[1].set_yticklabels(ylabels)
# ax[1].annotate('1e3', xy=(10300, 0), fontsize=30,annotation_clip=False)
ax[1].grid()
ax[1].legend(loc="lower right", prop={'size': 32},labelspacing=0,bbox_to_anchor=(1.02,-0.03))
# fig.text( 0.5, -0.02, 'Matrix Sizes',ha='center', fontsize=40) 
fig.savefig(f"step_wise_abft.pdf", bbox_inches='tight')