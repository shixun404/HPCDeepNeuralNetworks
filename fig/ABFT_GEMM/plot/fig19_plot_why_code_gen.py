...
ax[0].plot(N, kernel_[:l] / performance_scale, linewidth=ls, marker='P',markersize=ms, color = color[2], label="Our fused FT: off",clip_on=False)
ax[0].plot(N, kernel[:l] / performance_scale, '--' , linewidth=ls, marker='P',markersize=ms, color = color[2],label="Our codegen FT: off", clip_on=False)

ax[0].plot(N, abft_[:l] / performance_scale,linewidth=ls,  marker='s',markersize=ms,  color = color[3], label="Our fused FT: on",clip_on=False)
ax[0].plot(N, abft[:l] / performance_scale, '--', linewidth=ls, marker='s',markersize=ms,  color = color[3],label="Our codegen FT: on", clip_on=False)
print("\n\n################# M=N=K #####################")
...
# ax[0].legend(loc='lower right', fontsize=32,labelspacing = 0, bbox_to_anchor=(1.02,-0.04))
ax[0].legend(loc='lower right', fontsize=32,labelspacing = 0)
...
l = N.shape[0]
ax[1].plot(N, cublas[:l] / performance_scale, linewidth=ls,  marker='o', markersize=ms, color = color[1], label="cuBLAS", clip_on=False)
# ax[0].plot(N, abft_baseline[:l] / performance_scale,  marker='^',markersize=ms,  color = color[3], clip_on=False)

ax[1].plot(N, kernel_[:l] / performance_scale, linewidth=ls, marker='P',markersize=ms, color = color[2], label="Our fused FT: off",clip_on=False)
ax[1].plot(N, kernel[:l] / performance_scale, '--' ,linewidth=ls,  marker='P',markersize=ms, color = color[2],label="Our codegen FT: off", clip_on=False)

ax[1].plot(N, abft_[:l] / performance_scale, linewidth=ls,  marker='s',markersize=ms,  color = color[3], label="Our fused FT: on",clip_on=False)
ax[1].plot(N, abft[:l] / performance_scale, '--', linewidth=ls, marker='s',markersize=ms,  color = color[3],label="Our codegen FT: on", clip_on=False)
...
# ax[1].legend(loc='lower right', fontsize=32, labelspacing = 0, bbox_to_anchor=(1.02,-0.04))
ax[1].legend(loc='lower right', fontsize=32, labelspacing = 0)
ax[1].set_xlabel("(b) Matrix Sizes M=N, K = 256",fontdict=dict(weight='bold',size=40))
# ax[1].set_ylabel("Performance (GFLOPS)", fontdict=dict(weight='bold',size=40))
