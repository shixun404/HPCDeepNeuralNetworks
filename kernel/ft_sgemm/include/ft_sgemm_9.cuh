...
__global__  __launch_bounds__(256) void ft_sgemm_9(int N, float *A, float *B, float *C, float alpha, float beta){
    ...
    int r = 0, c = 0;
    float4 t[16], bb0[2],aa0[2],bb1[2], aa1[2], C1[16], pre_A, pre_B, C_c[2], C_r[2], C_c_ref[2], C_r_ref[2];
    ...
    // print_float4(C_r_ref[0], C_r_ref[1], gridDim.x * 128);
    // print_float4(C_c_ref[0], C_c_ref[1], gridDim.x * 128);
    __syncthreads();
    ...
    // C_r_ref[0] = t[0] + t[2] + t[4] + t[6] + t[8] + t[10] + t[12] + t[14];
    tcab(C_r_ref[0], C_r[0], -1.0, 1.0)
    tcab(C_r_ref[1], C_r[1], -1.0, 1.0)
    tcab(C_c_ref[0], C_c[0], -1.0, 1.0)
    tcab(C_c_ref[1], C_c[1], -1.0, 1.0)
    ...
    // // if(verified){
    if( ((tx&31) < 8 && (checksum_sum - err_bound > 0) || (checksum_sum + err_bound < 0))){             
    // if((tx&31) < 8){
        r = 0, c = 0;
