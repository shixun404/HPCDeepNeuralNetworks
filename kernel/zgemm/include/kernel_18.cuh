...
__global__ void zgemm_18(int M, int N, int K, double *A, double *B, double *C, double2 alpha, double2 beta){
    ...
    for(k = 0; k < K - 16; k += 8){
        ...
        b[1][1] = *(sB + offset_cur + ((wid % 4) * 16 + (tid % 32) / 4 + 1 * 8 + 64) * (4 + SKEW_KERNEL_2) + ((tid % 32) % 4));
    ...
    for(int i = 0; i < warp_col_tiles; ++i){
        #pragma unroll
        for(int j = 0; j < warp_row_tiles; ++j){
            mem_temp[0] = *(gC + (bid_x * 64 + (wid / 4) * 32 + j * 8 + ((tid % 32) / 4)) + (bid_y * 64 + (wid % 4) * 16 + i * 8 + ((tid % 32) % 4) * 2 + 0) * M);
            mem_temp[1] = *(gC + (bid_x * 64 + (wid / 4) * 32 + j * 8 + ((tid % 32) / 4)) + (bid_y * 64 + (wid % 4) * 16 + i * 8 + ((tid % 32) % 4) * 2 + 1) * M);
            #pragma unroll
            ...
            *(gC + (bid_x * 64 + (wid / 4) * 32 + j * 8 + ((tid % 32) / 4)) + (bid_y * 64 + (wid % 4) * 16 + i * 8 + ((tid % 32) % 4) * 2 + 0) * M) = mem_temp[0 + 2];
            *(gC + (bid_x * 64 + (wid / 4) * 32 + j * 8 + ((tid % 32) / 4)) + (bid_y * 64 + (wid % 4) * 16 + i * 8 + ((tid % 32) % 4) * 2 + 1) * M) = mem_temp[1 + 2];
