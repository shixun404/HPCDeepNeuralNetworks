...
__global__ void zgemm_14(int M, int N, int K, double *A, double *B, double *C, double2 alpha, double2 beta){
    int threadblock_tile_M = 32, threadblock_tile_N = 128;
    int tid = threadIdx.x;
    int bid_x = blockIdx.x, bid_y = blockIdx.y;
    int wid = tid / 32;
    
    
    __shared__ double shared_mem[((32 + SKEW_KERNEL_2) * 8 * 2 + (128 + SKEW_KERNEL_2) * 8 * 2 ) * 2];

    double2 * gA = (double2*)A;
    double2 * gB = (double2*)B;
    double2 * gC = (((double2*)C));

    double* sA_real = (shared_mem + (32 + SKEW_KERNEL_2) * 8 * 0);
    double* sA_imag = (shared_mem + (32 + SKEW_KERNEL_2) * 8 * 1);
    double* sB_real = (shared_mem + (128 + SKEW_KERNEL_2) * 8 * 0 + (32 + SKEW_KERNEL_2) * 8 * 2);
    double* sB_imag = (shared_mem + (128 + SKEW_KERNEL_2) * 8 * 1 + (32 + SKEW_KERNEL_2) * 8 * 2);
    ...
    wmma::fragment<wmma::matrix_a, 8, 8, 4, double, wmma::col_major> a_real_frag[2];
    wmma::fragment<wmma::matrix_a, 8, 8, 4, double, wmma::col_major> a_imag_frag[2];
    ...
    mem_temp[0] = *(gA + (bid_x * 32 + tid % 32) + (0 + tid / 32) * M);
    ...
    *(sB_imag + (tid / 128 + 6) * (128 + SKEW_KERNEL_2) + (tid % 128) + offset) = mem_temp[4].y;
    ...
    for(int k = 0; k < K; k += 8){
        ...
        offset = offset > 0 ? 0 : ((32 + SKEW_KERNEL_2 + 128 + SKEW_KERNEL_2) * 16);
    ...
    *(sB_imag + (tid / 128 + 6) * (128 + SKEW_KERNEL_2) + (tid % 128) + offset) = mem_temp[4].y;
    ...
    for(int i = 0; i < warp_col_tiles; ++i){
        #pragma unroll
        for(int j = 0; j < warp_row_tiles; ++j){
            mem_temp[0] = *(gC + (bid_x * 32 + (wid % 2) * 16 + j * 8 + ((tid % 32) / 4)) + (bid_y * 128 + (wid / 2) * 32 + i * 8 + ((tid % 32) % 4) * 2 + 0) * M);
            mem_temp[1] = *(gC + (bid_x * 32 + (wid % 2) * 16 + j * 8 + ((tid % 32) / 4)) + (bid_y * 128 + (wid / 2) * 32 + i * 8 + ((tid % 32) % 4) * 2 + 1) * M);
            #pragma unroll
            ...
            *(gC + (bid_x * 32 + (wid % 2) * 16 + j * 8 + ((tid % 32) / 4)) + (bid_y * 128 + (wid / 2) * 32 + i * 8 + ((tid % 32) % 4) * 2 + 0) * M) = mem_temp[0 + 2];
            *(gC + (bid_x * 32 + (wid % 2) * 16 + j * 8 + ((tid % 32) / 4)) + (bid_y * 128 + (wid / 2) * 32 + i * 8 + ((tid % 32) % 4) * 2 + 1) * M) = mem_temp[1 + 2];
