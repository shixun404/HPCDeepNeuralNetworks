...
__global__ void zgemm_12(int M, int N, int K, double *A, double *B, double *C, double2 alpha, double2 beta){
    ...
    // __shared__ double shared_mem[((64 + SKEW_KERNEL_2) * 8 * 2 + (128 + SKEW_KERNEL_2) * 8 * 2 ) * 2];
    ...
    double* sB_real = (shared_mem + (128 + SKEW_KERNEL_2) * 8 * 0 + (64 + SKEW_KERNEL_2) * 8 * 2);
    double* sB_imag = (shared_mem + (128 + SKEW_KERNEL_2) * 8 * 1 + (64 + SKEW_KERNEL_2) * 8 * 2);
    ...
    mem_temp[2] = *(gB + (0 + tid / 128) + (bid_y * 128 + tid % 128) * K);
    mem_temp[3] = *(gB + (0 + tid / 128 + 2) + (bid_y * 128 + tid % 128) * K);
    mem_temp[4] = *(gB + (0 + tid / 128 + 4) + (bid_y * 128 + tid % 128) * K);
    mem_temp[5] = *(gB + (0 + tid / 128 + 6) + (bid_y * 128 + tid % 128) * K);
    ...
    *(sB_real + (tid / 128) * (128 + SKEW_KERNEL_2) + (tid % 128) + offset) = mem_temp[2].x;
    *(sB_imag + (tid / 128) * (128 + SKEW_KERNEL_2) + (tid % 128) + offset) = mem_temp[2].y;
    *(sB_real + (tid / 128 + 2) * (128 + SKEW_KERNEL_2) + (tid % 128) + offset) = mem_temp[3].x;
    *(sB_imag + (tid / 128 + 2) * (128 + SKEW_KERNEL_2) + (tid % 128) + offset) = mem_temp[3].y;
    
    *(sB_real + (tid / 128 + 4) * (128 + SKEW_KERNEL_2) + (tid % 128) + offset) = mem_temp[4].x;
    *(sB_imag + (tid / 128 + 4) * (128 + SKEW_KERNEL_2) + (tid % 128) + offset) = mem_temp[4].y;
    *(sB_real + (tid / 128 + 6) * (128 + SKEW_KERNEL_2) + (tid % 128) + offset) = mem_temp[5].x;
    *(sB_imag + (tid / 128 + 6) * (128 + SKEW_KERNEL_2) + (tid % 128) + offset) = mem_temp[5].y;
    __syncthreads();
    ...
    for(int k = 0; k < K; k += 8){
        ...
        mem_temp[2] = *(gB + (k + 8 + tid / 128) + (bid_y * 128 + tid % 128) * K);
        mem_temp[3] = *(gB + (k + 8 + tid / 128 + 2) + (bid_y * 128 + tid % 128) * K);
        mem_temp[4] = *(gB + (k + 8 + tid / 128 + 4) + (bid_y * 128 + tid % 128) * K);
        mem_temp[5] = *(gB + (k + 8 + tid / 128 + 6) + (bid_y * 128 + tid % 128) * K);
        // #pragma unroll
        ...
        offset = offset > 0 ? 0 : ((64 + SKEW_KERNEL_2 + 128 + SKEW_KERNEL_2) * 16);
    ...
    *(sB_imag + (tid / 128 + 6) * (128 + SKEW_KERNEL_2) + (tid % 128) + offset) = mem_temp[5].y;
    ...
    for(int i = 0; i < warp_col_tiles; ++i){
        #pragma unroll
        for(int j = 0; j < warp_row_tiles; ++j){
            mem_temp[0] = *(gC + (bid_x * 64 + (wid % 2) * 32 + j * 8 + ((tid % 32) / 4)) + (bid_y * 128 + (wid / 2) * 32 + i * 8 + ((tid % 32) % 4) * 2 + 0) * M);
            mem_temp[1] = *(gC + (bid_x * 64 + (wid % 2) * 32 + j * 8 + ((tid % 32) / 4)) + (bid_y * 128 + (wid / 2) * 32 + i * 8 + ((tid % 32) % 4) * 2 + 1) * M);
            #pragma unroll
            ...
            *(gC + (bid_x * 64 + (wid % 2) * 32 + j * 8 + ((tid % 32) / 4)) + (bid_y * 128 + (wid / 2) * 32 + i * 8 + ((tid % 32) % 4) * 2 + 0) * M) = mem_temp[0 + 2];
            *(gC + (bid_x * 64 + (wid % 2) * 32 + j * 8 + ((tid % 32) / 4)) + (bid_y * 128 + (wid / 2) * 32 + i * 8 + ((tid % 32) % 4) * 2 + 1) * M) = mem_temp[1 + 2];
