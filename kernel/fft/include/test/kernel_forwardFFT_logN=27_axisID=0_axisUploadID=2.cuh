...
extern "C" __global__ void __launch_bounds__(1024) VkFFT_main_logN27_2 (float2* inputs, float2* outputs) {
...
float2* sdata = (float2*)shared;
	...
	mult.x = 0;
	mult.y = 0;
		disableThreads = ((((threadIdx.x + blockIdx.x * blockDim.x)) / 512) * (262144) < 134217728) ? 1 : 0;
		if(disableThreads>0) {
	...
	temp_6.y = temp_6.y + loc_0.y;
		...
		temp_7.x = w.x;
}		if ((((threadIdx.x + blockIdx.x * blockDim.x)) / 512) * (262144) < 134217728) {
		inoutID = ((threadIdx.x + blockIdx.x * blockDim.x)) % (512) + 512 * (threadIdx.y + 0) + (((threadIdx.x + blockIdx.x * blockDim.x)) / 512) * (262144);
			inoutID = (inoutID) * 1;
