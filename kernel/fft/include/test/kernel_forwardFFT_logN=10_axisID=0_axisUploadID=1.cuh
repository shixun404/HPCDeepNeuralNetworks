...
extern "C" __global__ void __launch_bounds__(128) VkFFT_main_logN10 (float2* inputs, float2* outputs) {
unsigned int sharedStride = 1040;
float2* sdata = (float2*)shared;
	...
	float angle=0;
		...
		inoutID = (combinedID % 1024) + (combinedID / 1024) * 1024;
			inoutID = (inoutID);
	...
	sdataID = inoutID + 0;
	sharedStride = 1088;	sdataID = (sdataID / 16) * 17 + sdataID % 16;	sdata[sdataID] = temp_0;
	sdataID = inoutID + 1;
	...
	inoutID = inoutID + stageInvocationID;
	sdataID = inoutID + 0;
	sharedStride = 1040;	sdata[sdataID] = temp_0;
	sdataID = inoutID + 64;
	...
	temp_0.x = temp_0.x + loc_0.x;
	temp_0.y = temp_0.y + loc_0.y;
		stageInvocationID = (threadIdx.x+ 128) % (512);
		angle = stageInvocationID * -6.13592315154256468e-03f;
		sdataID = threadIdx.x + 128;
	...
	temp_4.x = temp_4.x + loc_0.x;
	temp_4.y = temp_4.y + loc_0.y;
		stageInvocationID = (threadIdx.x+ 256) % (512);
		angle = stageInvocationID * -6.13592315154256468e-03f;
		sdataID = threadIdx.x + 256;
	...
	temp_2.x = temp_2.x + loc_0.x;
	temp_2.y = temp_2.y + loc_0.y;
		stageInvocationID = (threadIdx.x+ 384) % (512);
		angle = stageInvocationID * -6.13592315154256468e-03f;
		sdataID = threadIdx.x + 384;
