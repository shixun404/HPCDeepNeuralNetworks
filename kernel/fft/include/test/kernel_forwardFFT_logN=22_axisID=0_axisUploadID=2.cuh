...
extern "C" __global__ void __launch_bounds__(512) VkFFT_main_logN22_2 (float2* inputs, float2* outputs) {
...
float2* sdata = (float2*)shared;
	...
	float angle=0;
		disableThreads = (threadIdx.y * 2048 + (((blockIdx.x) % 1) * 2048 + ((blockIdx.x) / 1) * 8192) < 4194304) ? 1 : 0;
		if(disableThreads>0) {
	...
	sdata[sdataID] = temp_15;
		...
		if (((threadIdx.x + 128 * threadIdx.y) % 4 + ((blockIdx.x) / 1)*4 < 2048)){
		combinedID = (threadIdx.x + 128 * threadIdx.y) + 0;
		inoutID = combinedID % 4 + ((blockIdx.x) / 1)*4 + ((combinedID/4) * 2048)+ ((blockIdx.x) % 1) * 2048;
			inoutID = (inoutID) * 1;
