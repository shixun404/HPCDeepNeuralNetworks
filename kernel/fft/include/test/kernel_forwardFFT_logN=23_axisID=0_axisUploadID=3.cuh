...
extern "C" __global__ void __launch_bounds__(256) VkFFT_main_logN23_3 (float2* inputs, float2* outputs) {
...
float2* sdata = (float2*)shared;
	...
	float angle=0;
		unsigned int numActiveThreads = ((blockIdx.x/128)==16) ? 0 : 256;
		disableThreads = (threadIdx.x * 32768 + (((blockIdx.x) % 128) * 256 + ((blockIdx.x) / 128) * 524288) < 8388608) ? 1 : 0;
		if((threadIdx.x+16*threadIdx.y)< numActiveThreads) {
		combinedID = (threadIdx.x + 16 * threadIdx.y) + 0*numActiveThreads;
		inoutID = (combinedID % 256) + (combinedID / 256) * 32768 + (((blockIdx.x) % 128) * 256 + ((blockIdx.x) / 128) * 524288);
			inoutID = (inoutID);
	...
	sdata[sdataID] = temp_15;
		...
		if (((threadIdx.x + 16 * threadIdx.y) % 16 + ((blockIdx.x) / 128)*16 < 256)){
		combinedID = (threadIdx.x + 16 * threadIdx.y) + 0;
		inoutID = combinedID % 16 + ((blockIdx.x) / 128)*16 + ((combinedID/16) * 32768)+ ((blockIdx.x) % 128) * 256;
			inoutID = (inoutID) * 1;
