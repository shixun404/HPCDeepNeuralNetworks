...
__global__ void __launch_bounds__(128) fft_radix2_logN15_1(float2* inputs, float2* outputs) {
    ...
    temp_0 = inputs[(ty + 0 * 8) * gridDim.x * blockDim.x + tx + bx * 16];
    temp_1 = inputs[(ty + 1 * 8) * gridDim.x * blockDim.x + tx + bx * 16];
    temp_2 = inputs[(ty + 2 * 8) * gridDim.x * blockDim.x + tx + bx * 16];
    temp_3 = inputs[(ty + 3 * 8) * gridDim.x * blockDim.x + tx + bx * 16];
    temp_4 = inputs[(ty + 4 * 8) * gridDim.x * blockDim.x + tx + bx * 16];
    temp_5 = inputs[(ty + 5 * 8) * gridDim.x * blockDim.x + tx + bx * 16];
    temp_6 = inputs[(ty + 6 * 8) * gridDim.x * blockDim.x + tx + bx * 16];
    temp_7 = inputs[(ty + 7 * 8) * gridDim.x * blockDim.x + tx + bx * 16];
