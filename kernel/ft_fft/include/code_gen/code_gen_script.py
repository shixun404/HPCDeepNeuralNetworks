...
int main(int argc, char** argv){  
    ...
    '''
        ...
        '''
        ft_fft_script += f'''
                fft_radix2_logN{int(df['logN'][N-1])} <<<gridDim, blockDim, {int(df['sm_size_1'][N-1])}>>> ((float2*)input_d, (float2*)output_d, (float2*) checksum_r_d_{int(df['logN1'][N-1])});
                cudaDeviceSynchronize();
        '''
        ft_fft_script += '''
            }
    ...
    if(log_N == {N})''' + '''{
        ...
        ft_fft_script += f'''{{
                ...
                fft_radix2_logN{int(df['logN'][N-1])}_2 <<<gridDim, blockDim, {int(df['sm_size_2'][N-1])}>>> ((float2*)output_d_1, (float2*)output_d, (float2*) checksum_r_d_{int(df['logN2'][N-1])});
                cudaDeviceSynchronize();
            }}
        '''
        ft_fft_script += '''
