...
def ft_2D_fft_code_gen(N, N1, N2, num_block, num_thread,
    ...
    plan = []
    twiddle_type = []
    blockdim_x = num_thread // (N1 // signal_per_thread)
    blockdim_y = (N1 // signal_per_thread)
    i = 1
    while i <= N1 / radix:
        if i == N1 / radix:
...
'''
    ...
    int tx = threadIdx.x % {num_thread // (N1 // signal_per_thread)};
    int ty = threadIdx.x / {num_thread // (N1 // signal_per_thread)};
    int bx = blockIdx.x;
    int N = {N1};
...
'''
    ...
    temp_{order[signal_per_thread - offset + i]} = tmp;
    #if defined(LOG_ON)
    if(tx=={log_thread})printf("tx %d, rot_a.real %f,  rot_a.imag %f, local_id {i},  global_id %d, j %d, k %d, j * k %d, n_global %d, \\n",
                        tx, tmp_angle_rot.x, tmp_angle_rot.y, __id[{order[signal_per_thread - offset + i]}], j, k, j*k, n_global);
    #endif
    '''
