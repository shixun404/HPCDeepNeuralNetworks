...
__global__ void __launch_bounds__(256) fft_radix2_logN16_1(float2* inputs, float2* outputs, float2* r_1) {
...
__syncthreads();
    ...
    outputs[65536 + bx * 32 + 4 * (tid / 32) +  1].x = global_mem_checksum;
    ...
    outputs[65536 + bx * 32 + 4 * (tid / 32) +  2].x = global_mem_checksum;
    global_mem_checksum = 0;
    ...
    outputs[65536 + bx * 32 + 4 * (tid / 32) +  3].x = global_mem_checksum;
    global_mem_checksum = 0;
    ...
    outputs[65536 + bx * 32 + 4 * (tid / 32) +  4].x = global_mem_checksum;
    global_mem_checksum = 0;
