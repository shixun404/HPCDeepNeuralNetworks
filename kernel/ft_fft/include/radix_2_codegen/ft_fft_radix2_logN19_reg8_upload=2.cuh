extern __shared__ float shared[];
    ...
    float2 mem_checksum, mem_checksum_t1;
        ...
        *(float4*)(((float*)sdata) + tid * 8 + 1 * 4) = tmp_r;
        ...
        temp_0 = inputs[(ty + 0 * 128) + (tx + bx * 2) * 1024];
        temp_1 = inputs[(ty + 1 * 128) + (tx + bx * 2) * 1024];
        temp_2 = inputs[(ty + 2 * 128) + (tx + bx * 2) * 1024];
        temp_3 = inputs[(ty + 3 * 128) + (tx + bx * 2) * 1024];
        temp_4 = inputs[(ty + 4 * 128) + (tx + bx * 2) * 1024];
        temp_5 = inputs[(ty + 5 * 128) + (tx + bx * 2) * 1024];
        temp_6 = inputs[(ty + 6 * 128) + (tx + bx * 2) * 1024];
        temp_7 = inputs[(ty + 7 * 128) + (tx + bx * 2) * 1024];
    ...
    temp_7 = tmp;
        ...
        sdata[tx + 2 * __id[0]] = temp_0;
        
        sdata[tx + 2 * __id[4]] = temp_4;
        
        sdata[tx + 2 * __id[2]] = temp_2;
        
        sdata[tx + 2 * __id[6]] = temp_6;
        
        sdata[tx + 2 * __id[1]] = temp_1;
        
        sdata[tx + 2 * __id[5]] = temp_5;
        
        sdata[tx + 2 * __id[3]] = temp_3;
        
        sdata[tx + 2 * __id[7]] = temp_7;
        
        __syncthreads();		
        
        temp_0 = sdata[tx + 2 * (0 + ty)];
        __id[0] = ty + 0;
        
        temp_1 = sdata[tx + 2 * (128 + ty)];
        __id[1] = ty + 128;
        
        temp_2 = sdata[tx + 2 * (256 + ty)];
        __id[2] = ty + 256;
        
        temp_3 = sdata[tx + 2 * (384 + ty)];
        __id[3] = ty + 384;
        
        temp_4 = sdata[tx + 2 * (512 + ty)];
        __id[4] = ty + 512;
        
        temp_5 = sdata[tx + 2 * (640 + ty)];
        __id[5] = ty + 640;
        
        temp_6 = sdata[tx + 2 * (768 + ty)];
        __id[6] = ty + 768;
        
        temp_7 = sdata[tx + 2 * (896 + ty)];
        __id[7] = ty + 896;
    ...
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 8) * 0) / (float)(128.0), tmp_angle);
    MY_MUL(temp_0, tmp_angle, tmp);
    temp_0 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 8) * 1) / (float)(128.0), tmp_angle);
    MY_MUL(temp_4, tmp_angle, tmp);
    temp_4 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 8) * 2) / (float)(128.0), tmp_angle);
    MY_MUL(temp_2, tmp_angle, tmp);
    temp_2 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 8) * 3) / (float)(128.0), tmp_angle);
    MY_MUL(temp_6, tmp_angle, tmp);
    temp_6 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 8) * 4) / (float)(128.0), tmp_angle);
    MY_MUL(temp_1, tmp_angle, tmp);
    temp_1 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 8) * 5) / (float)(128.0), tmp_angle);
    MY_MUL(temp_5, tmp_angle, tmp);
    temp_5 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 8) * 6) / (float)(128.0), tmp_angle);
    MY_MUL(temp_3, tmp_angle, tmp);
    temp_3 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 8) * 7) / (float)(128.0), tmp_angle);
    MY_MUL(temp_7, tmp_angle, tmp);
    temp_7 = tmp;
    ...
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 64) * 0) / (float)(16.0), tmp_angle);
    MY_MUL(temp_0, tmp_angle, tmp);
    temp_0 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 64) * 1) / (float)(16.0), tmp_angle);
    MY_MUL(temp_4, tmp_angle, tmp);
    temp_4 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 64) * 2) / (float)(16.0), tmp_angle);
    MY_MUL(temp_2, tmp_angle, tmp);
    temp_2 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 64) * 3) / (float)(16.0), tmp_angle);
    MY_MUL(temp_6, tmp_angle, tmp);
    temp_6 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 64) * 4) / (float)(16.0), tmp_angle);
    MY_MUL(temp_1, tmp_angle, tmp);
    temp_1 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 64) * 5) / (float)(16.0), tmp_angle);
    MY_MUL(temp_5, tmp_angle, tmp);
    temp_5 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 64) * 6) / (float)(16.0), tmp_angle);
    MY_MUL(temp_3, tmp_angle, tmp);
    temp_3 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 64) * 7) / (float)(16.0), tmp_angle);
    MY_MUL(temp_7, tmp_angle, tmp);
    temp_7 = tmp;
