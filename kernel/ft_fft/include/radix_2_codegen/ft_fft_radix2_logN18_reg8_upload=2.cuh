extern __shared__ float shared[];
    __global__ void __launch_bounds__(64) fft_radix2_logN18_2(float2* inputs, float2* outputs, float2* r_1) {
    ...
    float2 mem_checksum, mem_checksum_t1;
        ...
        *(float4*)(((float*)sdata) + tid * 16 + 3 * 4) = tmp_r;
        ...
        temp_0 = inputs[(ty + 0 * 64) + (tx + bx * 1) * 512];
        temp_1 = inputs[(ty + 1 * 64) + (tx + bx * 1) * 512];
        temp_2 = inputs[(ty + 2 * 64) + (tx + bx * 1) * 512];
        temp_3 = inputs[(ty + 3 * 64) + (tx + bx * 1) * 512];
        temp_4 = inputs[(ty + 4 * 64) + (tx + bx * 1) * 512];
        temp_5 = inputs[(ty + 5 * 64) + (tx + bx * 1) * 512];
        temp_6 = inputs[(ty + 6 * 64) + (tx + bx * 1) * 512];
        temp_7 = inputs[(ty + 7 * 64) + (tx + bx * 1) * 512];
    ...
    temp_7 = tmp;
        ...
        temp_1 = sdata[tx + 1 * (64 + ty)];
        __id[1] = ty + 64;
        
        temp_2 = sdata[tx + 1 * (128 + ty)];
        __id[2] = ty + 128;
        
        temp_3 = sdata[tx + 1 * (192 + ty)];
        __id[3] = ty + 192;
        
        temp_4 = sdata[tx + 1 * (256 + ty)];
        __id[4] = ty + 256;
        
        temp_5 = sdata[tx + 1 * (320 + ty)];
        __id[5] = ty + 320;
        
        temp_6 = sdata[tx + 1 * (384 + ty)];
        __id[6] = ty + 384;
        
        temp_7 = sdata[tx + 1 * (448 + ty)];
        __id[7] = ty + 448;
    ...
    temp_7 = tmp;
        ...
        outputs[(tx + bx * 1) + 512 * __id[4]] = temp_4;
        
        outputs[(tx + bx * 1) + 512 * __id[2]] = temp_2;
        
        outputs[(tx + bx * 1) + 512 * __id[6]] = temp_6;
        
        outputs[(tx + bx * 1) + 512 * __id[1]] = temp_1;
        
        outputs[(tx + bx * 1) + 512 * __id[5]] = temp_5;
        
        outputs[(tx + bx * 1) + 512 * __id[3]] = temp_3;
        
        outputs[(tx + bx * 1) + 512 * __id[7]] = temp_7;
