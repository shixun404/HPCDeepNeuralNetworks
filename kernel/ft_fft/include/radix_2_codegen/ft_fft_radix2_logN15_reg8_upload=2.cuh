extern __shared__ float shared[];
    __global__ void __launch_bounds__(1024) fft_radix2_logN15_2(float2* inputs, float2* outputs, float2* r_1) {
    ...
    float2 mem_checksum, mem_checksum_t1;
        ...
        *(float1*)(((float*)sdata) + tid * 1) = tmp_r;
        ...
        temp_0 = inputs[(ty + 0 * 64) + (tx + bx * 16) * 512];
        temp_1 = inputs[(ty + 1 * 64) + (tx + bx * 16) * 512];
        temp_2 = inputs[(ty + 2 * 64) + (tx + bx * 16) * 512];
        temp_3 = inputs[(ty + 3 * 64) + (tx + bx * 16) * 512];
        temp_4 = inputs[(ty + 4 * 64) + (tx + bx * 16) * 512];
        temp_5 = inputs[(ty + 5 * 64) + (tx + bx * 16) * 512];
        temp_6 = inputs[(ty + 6 * 64) + (tx + bx * 16) * 512];
        temp_7 = inputs[(ty + 7 * 64) + (tx + bx * 16) * 512];
