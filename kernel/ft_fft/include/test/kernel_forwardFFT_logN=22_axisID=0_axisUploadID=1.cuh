...
extern "C" __global__ void __launch_bounds__(512) VkFFT_main_logN22_1 (float2* inputs, float2* outputs) {
...
float2* sdata = (float2*)shared;
	...
	mult.x = 0;
	mult.y = 0;
		disableThreads = ((((threadIdx.x + blockIdx.x * blockDim.x)) / 2048) * (4194304) < 4194304) ? 1 : 0;
		if(disableThreads>0) {
		inoutID = ((threadIdx.x + blockIdx.x * blockDim.x)) % (2048) + 2048 * (threadIdx.y + 0) + (((threadIdx.x + blockIdx.x * blockDim.x)) / 2048) * (4194304);
			inoutID = (inoutID);
			temp_0=inputs[inoutID];
		inoutID = ((threadIdx.x + blockIdx.x * blockDim.x)) % (2048) + 2048 * (threadIdx.y + 128) + (((threadIdx.x + blockIdx.x * blockDim.x)) / 2048) * (4194304);
			inoutID = (inoutID);
			temp_1=inputs[inoutID];
		inoutID = ((threadIdx.x + blockIdx.x * blockDim.x)) % (2048) + 2048 * (threadIdx.y + 256) + (((threadIdx.x + blockIdx.x * blockDim.x)) / 2048) * (4194304);
			inoutID = (inoutID);
			temp_2=inputs[inoutID];
		inoutID = ((threadIdx.x + blockIdx.x * blockDim.x)) % (2048) + 2048 * (threadIdx.y + 384) + (((threadIdx.x + blockIdx.x * blockDim.x)) / 2048) * (4194304);
			inoutID = (inoutID);
			temp_3=inputs[inoutID];
		inoutID = ((threadIdx.x + blockIdx.x * blockDim.x)) % (2048) + 2048 * (threadIdx.y + 512) + (((threadIdx.x + blockIdx.x * blockDim.x)) / 2048) * (4194304);
			inoutID = (inoutID);
			temp_4=inputs[inoutID];
		inoutID = ((threadIdx.x + blockIdx.x * blockDim.x)) % (2048) + 2048 * (threadIdx.y + 640) + (((threadIdx.x + blockIdx.x * blockDim.x)) / 2048) * (4194304);
			inoutID = (inoutID);
			temp_5=inputs[inoutID];
		inoutID = ((threadIdx.x + blockIdx.x * blockDim.x)) % (2048) + 2048 * (threadIdx.y + 768) + (((threadIdx.x + blockIdx.x * blockDim.x)) / 2048) * (4194304);
			inoutID = (inoutID);
			temp_6=inputs[inoutID];
		inoutID = ((threadIdx.x + blockIdx.x * blockDim.x)) % (2048) + 2048 * (threadIdx.y + 896) + (((threadIdx.x + blockIdx.x * blockDim.x)) / 2048) * (4194304);
			inoutID = (inoutID);
			temp_7=inputs[inoutID];
		inoutID = ((threadIdx.x + blockIdx.x * blockDim.x)) % (2048) + 2048 * (threadIdx.y + 1024) + (((threadIdx.x + blockIdx.x * blockDim.x)) / 2048) * (4194304);
			inoutID = (inoutID);
			temp_8=inputs[inoutID];
		inoutID = ((threadIdx.x + blockIdx.x * blockDim.x)) % (2048) + 2048 * (threadIdx.y + 1152) + (((threadIdx.x + blockIdx.x * blockDim.x)) / 2048) * (4194304);
			inoutID = (inoutID);
			temp_9=inputs[inoutID];
		inoutID = ((threadIdx.x + blockIdx.x * blockDim.x)) % (2048) + 2048 * (threadIdx.y + 1280) + (((threadIdx.x + blockIdx.x * blockDim.x)) / 2048) * (4194304);
			inoutID = (inoutID);
			temp_10=inputs[inoutID];
		inoutID = ((threadIdx.x + blockIdx.x * blockDim.x)) % (2048) + 2048 * (threadIdx.y + 1408) + (((threadIdx.x + blockIdx.x * blockDim.x)) / 2048) * (4194304);
			inoutID = (inoutID);
			temp_11=inputs[inoutID];
		inoutID = ((threadIdx.x + blockIdx.x * blockDim.x)) % (2048) + 2048 * (threadIdx.y + 1536) + (((threadIdx.x + blockIdx.x * blockDim.x)) / 2048) * (4194304);
			inoutID = (inoutID);
			temp_12=inputs[inoutID];
		inoutID = ((threadIdx.x + blockIdx.x * blockDim.x)) % (2048) + 2048 * (threadIdx.y + 1664) + (((threadIdx.x + blockIdx.x * blockDim.x)) / 2048) * (4194304);
			inoutID = (inoutID);
			temp_13=inputs[inoutID];
		inoutID = ((threadIdx.x + blockIdx.x * blockDim.x)) % (2048) + 2048 * (threadIdx.y + 1792) + (((threadIdx.x + blockIdx.x * blockDim.x)) / 2048) * (4194304);
			inoutID = (inoutID);
			temp_14=inputs[inoutID];
		inoutID = ((threadIdx.x + blockIdx.x * blockDim.x)) % (2048) + 2048 * (threadIdx.y + 1920) + (((threadIdx.x + blockIdx.x * blockDim.x)) / 2048) * (4194304);
			inoutID = (inoutID);
		if(threadIdx.y < 128){
	...
	sdata[sdataID] = temp_15;
		...
		temp_8 = sdata[sharedStride*(threadIdx.y+128)+threadIdx.x];
		temp_4 = sdata[sharedStride*(threadIdx.y+256)+threadIdx.x];
		temp_12 = sdata[sharedStride*(threadIdx.y+384)+threadIdx.x];
		temp_2 = sdata[sharedStride*(threadIdx.y+512)+threadIdx.x];
		temp_10 = sdata[sharedStride*(threadIdx.y+640)+threadIdx.x];
		temp_6 = sdata[sharedStride*(threadIdx.y+768)+threadIdx.x];
		temp_14 = sdata[sharedStride*(threadIdx.y+896)+threadIdx.x];
		temp_1 = sdata[sharedStride*(threadIdx.y+1024)+threadIdx.x];
		temp_9 = sdata[sharedStride*(threadIdx.y+1152)+threadIdx.x];
		temp_5 = sdata[sharedStride*(threadIdx.y+1280)+threadIdx.x];
		temp_13 = sdata[sharedStride*(threadIdx.y+1408)+threadIdx.x];
		temp_3 = sdata[sharedStride*(threadIdx.y+1536)+threadIdx.x];
		temp_11 = sdata[sharedStride*(threadIdx.y+1664)+threadIdx.x];
		temp_7 = sdata[sharedStride*(threadIdx.y+1792)+threadIdx.x];
		temp_15 = sdata[sharedStride*(threadIdx.y+1920)+threadIdx.x];
	w.x = __cosf(angle);
	w.y = __sinf(angle);
	loc_0.x = temp_1.x * w.x - temp_1.y * w.y;
	loc_0.y = temp_1.y * w.x + temp_1.x * w.y;
	...
	sdata[sdataID] = temp_15;
		...
		if(disableThreads>0) {
		stageInvocationID = (threadIdx.y+ 0) % (256);
		angle = stageInvocationID * -1.22718463030851294e-02f;
		temp_0 = sdata[sharedStride*(threadIdx.y+0)+threadIdx.x];
		temp_2 = sdata[sharedStride*(threadIdx.y+256)+threadIdx.x];
		temp_4 = sdata[sharedStride*(threadIdx.y+512)+threadIdx.x];
		temp_6 = sdata[sharedStride*(threadIdx.y+768)+threadIdx.x];
		temp_8 = sdata[sharedStride*(threadIdx.y+1024)+threadIdx.x];
		temp_10 = sdata[sharedStride*(threadIdx.y+1280)+threadIdx.x];
		temp_12 = sdata[sharedStride*(threadIdx.y+1536)+threadIdx.x];
		temp_14 = sdata[sharedStride*(threadIdx.y+1792)+threadIdx.x];
	w.x = __cosf(angle);
	w.y = __sinf(angle);
	loc_0.x = temp_8.x * w.x - temp_8.y * w.y;
	loc_0.y = temp_8.y * w.x + temp_8.x * w.y;
	...
	temp_12.x = temp_12.x + loc_0.x;
	temp_12.y = temp_12.y + loc_0.y;
		stageInvocationID = (threadIdx.y+ 128) % (256);
		angle = stageInvocationID * -1.22718463030851294e-02f;
		...
		temp_9 = sdata[sharedStride*(threadIdx.y+1152)+threadIdx.x];
		temp_11 = sdata[sharedStride*(threadIdx.y+1408)+threadIdx.x];
		temp_13 = sdata[sharedStride*(threadIdx.y+1664)+threadIdx.x];
		temp_15 = sdata[sharedStride*(threadIdx.y+1920)+threadIdx.x];
	w.x = __cosf(angle);
	w.y = __sinf(angle);
	loc_0.x = temp_9.x * w.x - temp_9.y * w.y;
	...
	temp_13.y = temp_13.y + loc_0.y;
		...
		angle = 2 * 3.14159265358979312e+00f * (((((threadIdx.x + blockIdx.x * blockDim.x)) % (2048)) * (threadIdx.y + 0)) / 4.19430400000000000e+06f);
		mult.x = __cosf(angle);
		mult.y = -__sinf(angle);
		w.x = temp_0.x * mult.x - temp_0.y * mult.y;
		temp_0.y = temp_0.y * mult.x + temp_0.x * mult.y;
		temp_0.x = w.x;
		angle = 2 * 3.14159265358979312e+00f * (((((threadIdx.x + blockIdx.x * blockDim.x)) % (2048)) * (threadIdx.y + 128)) / 4.19430400000000000e+06f);
		mult.x = __cosf(angle);
		mult.y = -__sinf(angle);
		w.x = temp_1.x * mult.x - temp_1.y * mult.y;
		temp_1.y = temp_1.y * mult.x + temp_1.x * mult.y;
		temp_1.x = w.x;
		angle = 2 * 3.14159265358979312e+00f * (((((threadIdx.x + blockIdx.x * blockDim.x)) % (2048)) * (threadIdx.y + 256)) / 4.19430400000000000e+06f);
		mult.x = __cosf(angle);
		mult.y = -__sinf(angle);
		w.x = temp_8.x * mult.x - temp_8.y * mult.y;
		temp_8.y = temp_8.y * mult.x + temp_8.x * mult.y;
		temp_8.x = w.x;
		angle = 2 * 3.14159265358979312e+00f * (((((threadIdx.x + blockIdx.x * blockDim.x)) % (2048)) * (threadIdx.y + 384)) / 4.19430400000000000e+06f);
		mult.x = __cosf(angle);
		mult.y = -__sinf(angle);
		w.x = temp_9.x * mult.x - temp_9.y * mult.y;
		temp_9.y = temp_9.y * mult.x + temp_9.x * mult.y;
		temp_9.x = w.x;
		angle = 2 * 3.14159265358979312e+00f * (((((threadIdx.x + blockIdx.x * blockDim.x)) % (2048)) * (threadIdx.y + 512)) / 4.19430400000000000e+06f);
		mult.x = __cosf(angle);
		mult.y = -__sinf(angle);
		w.x = temp_4.x * mult.x - temp_4.y * mult.y;
		temp_4.y = temp_4.y * mult.x + temp_4.x * mult.y;
		temp_4.x = w.x;
		angle = 2 * 3.14159265358979312e+00f * (((((threadIdx.x + blockIdx.x * blockDim.x)) % (2048)) * (threadIdx.y + 640)) / 4.19430400000000000e+06f);
		mult.x = __cosf(angle);
		mult.y = -__sinf(angle);
		w.x = temp_5.x * mult.x - temp_5.y * mult.y;
		temp_5.y = temp_5.y * mult.x + temp_5.x * mult.y;
		temp_5.x = w.x;
		angle = 2 * 3.14159265358979312e+00f * (((((threadIdx.x + blockIdx.x * blockDim.x)) % (2048)) * (threadIdx.y + 768)) / 4.19430400000000000e+06f);
		mult.x = __cosf(angle);
		mult.y = -__sinf(angle);
		w.x = temp_12.x * mult.x - temp_12.y * mult.y;
		temp_12.y = temp_12.y * mult.x + temp_12.x * mult.y;
		temp_12.x = w.x;
		angle = 2 * 3.14159265358979312e+00f * (((((threadIdx.x + blockIdx.x * blockDim.x)) % (2048)) * (threadIdx.y + 896)) / 4.19430400000000000e+06f);
		mult.x = __cosf(angle);
		mult.y = -__sinf(angle);
		w.x = temp_13.x * mult.x - temp_13.y * mult.y;
		temp_13.y = temp_13.y * mult.x + temp_13.x * mult.y;
		temp_13.x = w.x;
		angle = 2 * 3.14159265358979312e+00f * (((((threadIdx.x + blockIdx.x * blockDim.x)) % (2048)) * (threadIdx.y + 1024)) / 4.19430400000000000e+06f);
		mult.x = __cosf(angle);
		mult.y = -__sinf(angle);
		w.x = temp_2.x * mult.x - temp_2.y * mult.y;
		temp_2.y = temp_2.y * mult.x + temp_2.x * mult.y;
		temp_2.x = w.x;
		angle = 2 * 3.14159265358979312e+00f * (((((threadIdx.x + blockIdx.x * blockDim.x)) % (2048)) * (threadIdx.y + 1152)) / 4.19430400000000000e+06f);
		mult.x = __cosf(angle);
		mult.y = -__sinf(angle);
		w.x = temp_3.x * mult.x - temp_3.y * mult.y;
		temp_3.y = temp_3.y * mult.x + temp_3.x * mult.y;
		temp_3.x = w.x;
		angle = 2 * 3.14159265358979312e+00f * (((((threadIdx.x + blockIdx.x * blockDim.x)) % (2048)) * (threadIdx.y + 1280)) / 4.19430400000000000e+06f);
		mult.x = __cosf(angle);
		mult.y = -__sinf(angle);
		w.x = temp_10.x * mult.x - temp_10.y * mult.y;
		temp_10.y = temp_10.y * mult.x + temp_10.x * mult.y;
		temp_10.x = w.x;
		angle = 2 * 3.14159265358979312e+00f * (((((threadIdx.x + blockIdx.x * blockDim.x)) % (2048)) * (threadIdx.y + 1408)) / 4.19430400000000000e+06f);
		mult.x = __cosf(angle);
		mult.y = -__sinf(angle);
		w.x = temp_11.x * mult.x - temp_11.y * mult.y;
		temp_11.y = temp_11.y * mult.x + temp_11.x * mult.y;
		temp_11.x = w.x;
		angle = 2 * 3.14159265358979312e+00f * (((((threadIdx.x + blockIdx.x * blockDim.x)) % (2048)) * (threadIdx.y + 1536)) / 4.19430400000000000e+06f);
		mult.x = __cosf(angle);
		mult.y = -__sinf(angle);
		w.x = temp_6.x * mult.x - temp_6.y * mult.y;
		temp_6.y = temp_6.y * mult.x + temp_6.x * mult.y;
		temp_6.x = w.x;
		angle = 2 * 3.14159265358979312e+00f * (((((threadIdx.x + blockIdx.x * blockDim.x)) % (2048)) * (threadIdx.y + 1664)) / 4.19430400000000000e+06f);
		mult.x = __cosf(angle);
		mult.y = -__sinf(angle);
		w.x = temp_7.x * mult.x - temp_7.y * mult.y;
		temp_7.y = temp_7.y * mult.x + temp_7.x * mult.y;
		temp_7.x = w.x;
		angle = 2 * 3.14159265358979312e+00f * (((((threadIdx.x + blockIdx.x * blockDim.x)) % (2048)) * (threadIdx.y + 1792)) / 4.19430400000000000e+06f);
		mult.x = __cosf(angle);
		mult.y = -__sinf(angle);
		w.x = temp_14.x * mult.x - temp_14.y * mult.y;
		temp_14.y = temp_14.y * mult.x + temp_14.x * mult.y;
		temp_14.x = w.x;
		angle = 2 * 3.14159265358979312e+00f * (((((threadIdx.x + blockIdx.x * blockDim.x)) % (2048)) * (threadIdx.y + 1920)) / 4.19430400000000000e+06f);
		mult.x = __cosf(angle);
		mult.y = -__sinf(angle);
		w.x = temp_15.x * mult.x - temp_15.y * mult.y;
		temp_15.y = temp_15.y * mult.x + temp_15.x * mult.y;
		temp_15.x = w.x;
}		if ((((threadIdx.x + blockIdx.x * blockDim.x)) / 2048) * (4194304) < 4194304) {
		inoutID = ((threadIdx.x + blockIdx.x * blockDim.x)) % (2048) + 2048 * (threadIdx.y + 0) + (((threadIdx.x + blockIdx.x * blockDim.x)) / 2048) * (4194304);
			inoutID = (inoutID) * 1;
