...
extern "C" __global__ void __launch_bounds__(256) VkFFT_main_logN17_1 (float2* inputs, float2* outputs) {
...
float2* sdata = (float2*)shared;
	...
	mult.x = 0;
	mult.y = 0;
		disableThreads = ((((threadIdx.x + blockIdx.x * blockDim.x)) / 512) * (131072) < 131072) ? 1 : 0;
		if(disableThreads>0) {
	...
	temp_7.y = temp_7.y + loc_0.y;
		...
		temp_15.x = w.x;
}		if ((((threadIdx.x + blockIdx.x * blockDim.x)) / 512) * (131072) < 131072) {
		inoutID = ((threadIdx.x + blockIdx.x * blockDim.x)) % (512) + 512 * (threadIdx.y + 0) + (((threadIdx.x + blockIdx.x * blockDim.x)) / 512) * (131072);
			inoutID = (inoutID) * 1;
