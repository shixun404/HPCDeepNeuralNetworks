...
extern "C" __global__ void __launch_bounds__(1024) VkFFT_main_logN26_3 (float2* inputs, float2* outputs) {
...
float2* sdata = (float2*)shared;
	...
	float angle=0;
		disableThreads = (threadIdx.y * 131072 + (((blockIdx.x) % 256) * 512 + ((blockIdx.x) / 256) * 2097152) < 67108864) ? 1 : 0;
		if(disableThreads>0) {
		inoutID = threadIdx.x+0+threadIdx.y * 131072 + (((blockIdx.x) % 256) * 512 + ((blockIdx.x) / 256) * 2097152);
			inoutID = (inoutID);
		sdata[sharedStride*threadIdx.y + (threadIdx.x + 0)] = inputs[inoutID];
		inoutID = threadIdx.x+64+threadIdx.y * 131072 + (((blockIdx.x) % 256) * 512 + ((blockIdx.x) / 256) * 2097152);
			inoutID = (inoutID);
		sdata[sharedStride*threadIdx.y + (threadIdx.x + 64)] = inputs[inoutID];
		inoutID = threadIdx.x+128+threadIdx.y * 131072 + (((blockIdx.x) % 256) * 512 + ((blockIdx.x) / 256) * 2097152);
			inoutID = (inoutID);
		sdata[sharedStride*threadIdx.y + (threadIdx.x + 128)] = inputs[inoutID];
		inoutID = threadIdx.x+192+threadIdx.y * 131072 + (((blockIdx.x) % 256) * 512 + ((blockIdx.x) / 256) * 2097152);
			inoutID = (inoutID);
		sdata[sharedStride*threadIdx.y + (threadIdx.x + 192)] = inputs[inoutID];
		inoutID = threadIdx.x+256+threadIdx.y * 131072 + (((blockIdx.x) % 256) * 512 + ((blockIdx.x) / 256) * 2097152);
			inoutID = (inoutID);
		sdata[sharedStride*threadIdx.y + (threadIdx.x + 256)] = inputs[inoutID];
		inoutID = threadIdx.x+320+threadIdx.y * 131072 + (((blockIdx.x) % 256) * 512 + ((blockIdx.x) / 256) * 2097152);
			inoutID = (inoutID);
		sdata[sharedStride*threadIdx.y + (threadIdx.x + 320)] = inputs[inoutID];
		inoutID = threadIdx.x+384+threadIdx.y * 131072 + (((blockIdx.x) % 256) * 512 + ((blockIdx.x) / 256) * 2097152);
			inoutID = (inoutID);
		sdata[sharedStride*threadIdx.y + (threadIdx.x + 384)] = inputs[inoutID];
		inoutID = threadIdx.x+448+threadIdx.y * 131072 + (((blockIdx.x) % 256) * 512 + ((blockIdx.x) / 256) * 2097152);
			inoutID = (inoutID);
	...
	sdata[sdataID] = temp_7;
		...
		if (((threadIdx.x + 64 * threadIdx.y) % 16 + ((blockIdx.x) / 256)*16 < 512)){
		combinedID = (threadIdx.x + 64 * threadIdx.y) + 0;
		inoutID = combinedID % 16 + ((blockIdx.x) / 256)*16 + ((combinedID/16) * 131072)+ ((blockIdx.x) % 256) * 512;
			inoutID = (inoutID) * 1;
