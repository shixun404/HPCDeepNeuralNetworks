...
__global__ void __launch_bounds__(1024) fft_logN19_1(float2 *inputs, float2 *outputs){
    ...
    temp[0] = inputs[(block_read_id + tx) + (ty + 0 * 64) * 1024];
    temp[1] = inputs[(block_read_id + tx) + (ty + 1 * 64) * 1024];
    temp[2] = inputs[(block_read_id + tx) + (ty + 2 * 64) * 1024];
    temp[3] = inputs[(block_read_id + tx) + (ty + 3 * 64) * 1024];
    temp[4] = inputs[(block_read_id + tx) + (ty + 4 * 64) * 1024];
    temp[5] = inputs[(block_read_id + tx) + (ty + 5 * 64) * 1024];
    temp[6] = inputs[(block_read_id + tx) + (ty + 6 * 64) * 1024];
    temp[7] = inputs[(block_read_id + tx) + (ty + 7 * 64) * 1024];
	...
	for(int i = 0; i < 8; ++i){
		...
		if(tx==0 && bx==0 && ty==0)printf("tx %d, __id[%d] = %d\n", tx,  i, __id[8-offset + i]);
		#endif
        MY_ANGLE2COMPLEX((float)(-M_PI * 2 * (tx + bx * blockDim.x) * (__id[8-offset + i])) / (float)(1024 * 512), tmp_angle);
		MY_MUL(temp[8 - offset + i], tmp_angle, tmp);
        temp[8 - offset + i] = tmp;
		outputs[(tx + bx * blockDim.x) + 1024 * __id[8 - offset + i]] = temp[8 - offset + i];
    }
}