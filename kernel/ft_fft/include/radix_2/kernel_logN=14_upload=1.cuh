...
__global__ void __launch_bounds__(128) fft_logN14_1(float2 *inputs, float2 *outputs){
    ...
    temp[0] = inputs[(block_read_id + tx) + (ty +  0) * 256];
    temp[1] = inputs[(block_read_id + tx) + (ty +  8) * 256];
    temp[2] = inputs[(block_read_id + tx) + (ty + 16) * 256];
    temp[3] = inputs[(block_read_id + tx) + (ty + 24) * 256];
    temp[4] = inputs[(block_read_id + tx) + (ty + 32) * 256];
    temp[5] = inputs[(block_read_id + tx) + (ty + 40) * 256];
    temp[6] = inputs[(block_read_id + tx) + (ty + 48) * 256];
    temp[7] = inputs[(block_read_id + tx) + (ty + 56) * 256];
	...
	for(int i = 0; i < 8; ++i){
		...
		if(tx==0 && bx==0 && ty==0)printf("tx %d, __id[%d] = %d\n", tx,  i, __id[8-offset + i]);
		#endif
        MY_ANGLE2COMPLEX((float)(-M_PI * 2 * (tx + bx * blockDim.x) * (__id[8-offset + i])) / (float)(16384), tmp_angle);
		MY_MUL(temp[8 - offset + i], tmp_angle, tmp);
        temp[8 - offset + i] = tmp;
		outputs[(tx + bx * blockDim.x) + 256 * __id[8 - offset + i]] = temp[8 - offset + i];
    }
