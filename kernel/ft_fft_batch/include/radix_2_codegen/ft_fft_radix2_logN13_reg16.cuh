extern __shared__ float shared[];
    ...
    __global__ void __launch_bounds__(512) fft_radix2_logN13(float2* inputs, float2* outputs, float2* r_2) {
    
    // r_1 = constData;
    // float2* r_1 = r_2 + 8192 * (blockIdx.x % 1);
    float2* r_1 = r_2;
    float2 r[3];
    r[0].x = 1.0f;
    r[0].y = 0.0f;
    ...
    float2 temp_0;
        ...
        tmp_r = *(float4*)(((float*)r_1) + tid * 32 + 0 * 4);
        *(float4*)(((float*)sdata) + tid * 32 + 0 * 4) = tmp_r;
        // if(bx == 0)printf("%d, hello\n", tid);
        
        tmp_r = *(float4*)(((float*)r_1) + tid * 32 + 1 * 4);
        *(float4*)(((float*)sdata) + tid * 32 + 1 * 4) = tmp_r;
        // if(bx == 0)printf("%d, hello\n", tid);
        
        tmp_r = *(float4*)(((float*)r_1) + tid * 32 + 2 * 4);
        *(float4*)(((float*)sdata) + tid * 32 + 2 * 4) = tmp_r;
        // if(bx == 0)printf("%d, hello\n", tid);
        
        tmp_r = *(float4*)(((float*)r_1) + tid * 32 + 3 * 4);
        *(float4*)(((float*)sdata) + tid * 32 + 3 * 4) = tmp_r;
        // if(bx == 0)printf("%d, hello\n", tid);
        
        tmp_r = *(float4*)(((float*)r_1) + tid * 32 + 4 * 4);
        *(float4*)(((float*)sdata) + tid * 32 + 4 * 4) = tmp_r;
        // if(bx == 0)printf("%d, hello\n", tid);
        
        tmp_r = *(float4*)(((float*)r_1) + tid * 32 + 5 * 4);
        *(float4*)(((float*)sdata) + tid * 32 + 5 * 4) = tmp_r;
        // if(bx == 0)printf("%d, hello\n", tid);
        
        tmp_r = *(float4*)(((float*)r_1) + tid * 32 + 6 * 4);
        *(float4*)(((float*)sdata) + tid * 32 + 6 * 4) = tmp_r;
        // if(bx == 0)printf("%d, hello\n", tid);
        
        tmp_r = *(float4*)(((float*)r_1) + tid * 32 + 7 * 4);
        *(float4*)(((float*)sdata) + tid * 32 + 7 * 4) = tmp_r;
        // if(bx == 0)printf("%d, hello\n", tid);
        
        #endif
        
        temp_0 = inputs[(ty + 0 * 512) + (tx + bx * 1) * 8192];
        temp_1 = inputs[(ty + 1 * 512) + (tx + bx * 1) * 8192];
        temp_2 = inputs[(ty + 2 * 512) + (tx + bx * 1) * 8192];
        temp_3 = inputs[(ty + 3 * 512) + (tx + bx * 1) * 8192];
        temp_4 = inputs[(ty + 4 * 512) + (tx + bx * 1) * 8192];
        temp_5 = inputs[(ty + 5 * 512) + (tx + bx * 1) * 8192];
        temp_6 = inputs[(ty + 6 * 512) + (tx + bx * 1) * 8192];
        temp_7 = inputs[(ty + 7 * 512) + (tx + bx * 1) * 8192];
        temp_8 = inputs[(ty + 8 * 512) + (tx + bx * 1) * 8192];
        temp_9 = inputs[(ty + 9 * 512) + (tx + bx * 1) * 8192];
        temp_10 = inputs[(ty + 10 * 512) + (tx + bx * 1) * 8192];
        temp_11 = inputs[(ty + 11 * 512) + (tx + bx * 1) * 8192];
        temp_12 = inputs[(ty + 12 * 512) + (tx + bx * 1) * 8192];
        temp_13 = inputs[(ty + 13 * 512) + (tx + bx * 1) * 8192];
        temp_14 = inputs[(ty + 14 * 512) + (tx + bx * 1) * 8192];
        temp_15 = inputs[(ty + 15 * 512) + (tx + bx * 1) * 8192];
    ...
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 1) * 0) / (float)(8192), tmp_angle);
    MY_MUL(temp_0, tmp_angle, tmp);
    temp_0 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 1) * 1) / (float)(8192), tmp_angle);
    MY_MUL(temp_8, tmp_angle, tmp);
    temp_8 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 1) * 2) / (float)(8192), tmp_angle);
    MY_MUL(temp_4, tmp_angle, tmp);
    temp_4 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 1) * 3) / (float)(8192), tmp_angle);
    MY_MUL(temp_12, tmp_angle, tmp);
    temp_12 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 1) * 4) / (float)(8192), tmp_angle);
    MY_MUL(temp_2, tmp_angle, tmp);
    temp_2 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 1) * 5) / (float)(8192), tmp_angle);
    MY_MUL(temp_10, tmp_angle, tmp);
    temp_10 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 1) * 6) / (float)(8192), tmp_angle);
    MY_MUL(temp_6, tmp_angle, tmp);
    temp_6 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 1) * 7) / (float)(8192), tmp_angle);
    MY_MUL(temp_14, tmp_angle, tmp);
    temp_14 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 1) * 8) / (float)(8192), tmp_angle);
    MY_MUL(temp_1, tmp_angle, tmp);
    temp_1 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 1) * 9) / (float)(8192), tmp_angle);
    MY_MUL(temp_9, tmp_angle, tmp);
    temp_9 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 1) * 10) / (float)(8192), tmp_angle);
    MY_MUL(temp_5, tmp_angle, tmp);
    temp_5 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 1) * 11) / (float)(8192), tmp_angle);
    MY_MUL(temp_13, tmp_angle, tmp);
    temp_13 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 1) * 12) / (float)(8192), tmp_angle);
    MY_MUL(temp_3, tmp_angle, tmp);
    temp_3 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 1) * 13) / (float)(8192), tmp_angle);
    MY_MUL(temp_11, tmp_angle, tmp);
    temp_11 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 1) * 14) / (float)(8192), tmp_angle);
    MY_MUL(temp_7, tmp_angle, tmp);
    temp_7 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 1) * 15) / (float)(8192), tmp_angle);
    MY_MUL(temp_15, tmp_angle, tmp);
    temp_15 = tmp;
    
        sdata[tx + 1 * __id[0]] = temp_0;
        
        sdata[tx + 1 * __id[8]] = temp_8;
        
        sdata[tx + 1 * __id[4]] = temp_4;
        
        sdata[tx + 1 * __id[12]] = temp_12;
        
        sdata[tx + 1 * __id[2]] = temp_2;
        
        sdata[tx + 1 * __id[10]] = temp_10;
        
        sdata[tx + 1 * __id[6]] = temp_6;
        
        sdata[tx + 1 * __id[14]] = temp_14;
        
        sdata[tx + 1 * __id[1]] = temp_1;
        
        sdata[tx + 1 * __id[9]] = temp_9;
        
        sdata[tx + 1 * __id[5]] = temp_5;
        
        sdata[tx + 1 * __id[13]] = temp_13;
        
        sdata[tx + 1 * __id[3]] = temp_3;
        
        sdata[tx + 1 * __id[11]] = temp_11;
        
        sdata[tx + 1 * __id[7]] = temp_7;
        
        sdata[tx + 1 * __id[15]] = temp_15;
        
        __syncthreads();		
        
        temp_0 = sdata[tx + 1 * (0 + ty)];
        __id[0] = ty + 0;
        
        temp_1 = sdata[tx + 1 * (512 + ty)];
        __id[1] = ty + 512;
        
        temp_2 = sdata[tx + 1 * (1024 + ty)];
        __id[2] = ty + 1024;
        
        temp_3 = sdata[tx + 1 * (1536 + ty)];
        __id[3] = ty + 1536;
        
        temp_4 = sdata[tx + 1 * (2048 + ty)];
        __id[4] = ty + 2048;
        
        temp_5 = sdata[tx + 1 * (2560 + ty)];
        __id[5] = ty + 2560;
        
        temp_6 = sdata[tx + 1 * (3072 + ty)];
        __id[6] = ty + 3072;
        
        temp_7 = sdata[tx + 1 * (3584 + ty)];
        __id[7] = ty + 3584;
        
        temp_8 = sdata[tx + 1 * (4096 + ty)];
        __id[8] = ty + 4096;
        
        temp_9 = sdata[tx + 1 * (4608 + ty)];
        __id[9] = ty + 4608;
        
        temp_10 = sdata[tx + 1 * (5120 + ty)];
        __id[10] = ty + 5120;
        
        temp_11 = sdata[tx + 1 * (5632 + ty)];
        __id[11] = ty + 5632;
        
        temp_12 = sdata[tx + 1 * (6144 + ty)];
        __id[12] = ty + 6144;
        
        temp_13 = sdata[tx + 1 * (6656 + ty)];
        __id[13] = ty + 6656;
        
        temp_14 = sdata[tx + 1 * (7168 + ty)];
        __id[14] = ty + 7168;
        
        temp_15 = sdata[tx + 1 * (7680 + ty)];
        __id[15] = ty + 7680;
    ...
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 16) * 0) / (float)(512.0), tmp_angle);
    MY_MUL(temp_0, tmp_angle, tmp);
    temp_0 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 16) * 1) / (float)(512.0), tmp_angle);
    MY_MUL(temp_8, tmp_angle, tmp);
    temp_8 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 16) * 2) / (float)(512.0), tmp_angle);
    MY_MUL(temp_4, tmp_angle, tmp);
    temp_4 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 16) * 3) / (float)(512.0), tmp_angle);
    MY_MUL(temp_12, tmp_angle, tmp);
    temp_12 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 16) * 4) / (float)(512.0), tmp_angle);
    MY_MUL(temp_2, tmp_angle, tmp);
    temp_2 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 16) * 5) / (float)(512.0), tmp_angle);
    MY_MUL(temp_10, tmp_angle, tmp);
    temp_10 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 16) * 6) / (float)(512.0), tmp_angle);
    MY_MUL(temp_6, tmp_angle, tmp);
    temp_6 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 16) * 7) / (float)(512.0), tmp_angle);
    MY_MUL(temp_14, tmp_angle, tmp);
    temp_14 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 16) * 8) / (float)(512.0), tmp_angle);
    MY_MUL(temp_1, tmp_angle, tmp);
    temp_1 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 16) * 9) / (float)(512.0), tmp_angle);
    MY_MUL(temp_9, tmp_angle, tmp);
    temp_9 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 16) * 10) / (float)(512.0), tmp_angle);
    MY_MUL(temp_5, tmp_angle, tmp);
    temp_5 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 16) * 11) / (float)(512.0), tmp_angle);
    MY_MUL(temp_13, tmp_angle, tmp);
    temp_13 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 16) * 12) / (float)(512.0), tmp_angle);
    MY_MUL(temp_3, tmp_angle, tmp);
    temp_3 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 16) * 13) / (float)(512.0), tmp_angle);
    MY_MUL(temp_11, tmp_angle, tmp);
    temp_11 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 16) * 14) / (float)(512.0), tmp_angle);
    MY_MUL(temp_7, tmp_angle, tmp);
    temp_7 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 16) * 15) / (float)(512.0), tmp_angle);
    MY_MUL(temp_15, tmp_angle, tmp);
    temp_15 = tmp;
    ...
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 256) * 0) / (float)(32.0), tmp_angle);
    MY_MUL(temp_0, tmp_angle, tmp);
    temp_0 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 256) * 1) / (float)(32.0), tmp_angle);
    MY_MUL(temp_8, tmp_angle, tmp);
    temp_8 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 256) * 2) / (float)(32.0), tmp_angle);
    MY_MUL(temp_4, tmp_angle, tmp);
    temp_4 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 256) * 3) / (float)(32.0), tmp_angle);
    MY_MUL(temp_12, tmp_angle, tmp);
    temp_12 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 256) * 4) / (float)(32.0), tmp_angle);
    MY_MUL(temp_2, tmp_angle, tmp);
    temp_2 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 256) * 5) / (float)(32.0), tmp_angle);
    MY_MUL(temp_10, tmp_angle, tmp);
    temp_10 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 256) * 6) / (float)(32.0), tmp_angle);
    MY_MUL(temp_6, tmp_angle, tmp);
    temp_6 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 256) * 7) / (float)(32.0), tmp_angle);
    MY_MUL(temp_14, tmp_angle, tmp);
    temp_14 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 256) * 8) / (float)(32.0), tmp_angle);
    MY_MUL(temp_1, tmp_angle, tmp);
    temp_1 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 256) * 9) / (float)(32.0), tmp_angle);
    MY_MUL(temp_9, tmp_angle, tmp);
    temp_9 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 256) * 10) / (float)(32.0), tmp_angle);
    MY_MUL(temp_5, tmp_angle, tmp);
    temp_5 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 256) * 11) / (float)(32.0), tmp_angle);
    MY_MUL(temp_13, tmp_angle, tmp);
    temp_13 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 256) * 12) / (float)(32.0), tmp_angle);
    MY_MUL(temp_3, tmp_angle, tmp);
    temp_3 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 256) * 13) / (float)(32.0), tmp_angle);
    MY_MUL(temp_11, tmp_angle, tmp);
    temp_11 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 256) * 14) / (float)(32.0), tmp_angle);
    MY_MUL(temp_7, tmp_angle, tmp);
    temp_7 = tmp;
    
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / 256) * 15) / (float)(32.0), tmp_angle);
    MY_MUL(temp_15, tmp_angle, tmp);
    temp_15 = tmp;
        ...
        outputs[(tx + bx * 1) * 8192 +  __id[0]] = temp_0;
        
        outputs[(tx + bx * 1) * 8192 +  __id[1]] = temp_1;
        
        outputs[(tx + bx * 1) * 8192 +  __id[2]] = temp_2;
        
        outputs[(tx + bx * 1) * 8192 +  __id[3]] = temp_3;
        
        outputs[(tx + bx * 1) * 8192 +  __id[4]] = temp_4;
        
        outputs[(tx + bx * 1) * 8192 +  __id[5]] = temp_5;
        
        outputs[(tx + bx * 1) * 8192 +  __id[6]] = temp_6;
        
        outputs[(tx + bx * 1) * 8192 +  __id[7]] = temp_7;
        
        outputs[(tx + bx * 1) * 8192 +  __id[8]] = temp_8;
        
        outputs[(tx + bx * 1) * 8192 +  __id[9]] = temp_9;
        
        outputs[(tx + bx * 1) * 8192 +  __id[10]] = temp_10;
        
        outputs[(tx + bx * 1) * 8192 +  __id[11]] = temp_11;
        
        outputs[(tx + bx * 1) * 8192 +  __id[12]] = temp_12;
        
        outputs[(tx + bx * 1) * 8192 +  __id[13]] = temp_13;
        
        outputs[(tx + bx * 1) * 8192 +  __id[14]] = temp_14;
        
        outputs[(tx + bx * 1) * 8192 +  __id[15]] = temp_15;
