...
__global__ void __launch_bounds__(128) fft_radix2_logN15_2(float2* inputs, float2* outputs, float2* r_1) {

    int batch_id = (blockIdx.x * blockDim.x) / 128;
float2 r[3];
r[0].x = 1.0f;
r[0].y = 0.0f;
r[1].x = -0.5f;
...
float2 mem_checksum, mem_checksum_t1;
    ...
    *(float4*)(((float*)sdata) + tid * 4) = tmp_r;
    ...
    temp_0 = inputs[(ty + 0 * 16) + ((tx + bx * 8) % 128) * 256 + batch_id * 32768];
    temp_1 = inputs[(ty + 1 * 16) + ((tx + bx * 8) % 128) * 256 + batch_id * 32768];
    temp_2 = inputs[(ty + 2 * 16) + ((tx + bx * 8) % 128) * 256 + batch_id * 32768];
    temp_3 = inputs[(ty + 3 * 16) + ((tx + bx * 8) % 128) * 256 + batch_id * 32768];
    temp_4 = inputs[(ty + 4 * 16) + ((tx + bx * 8) % 128) * 256 + batch_id * 32768];
    temp_5 = inputs[(ty + 5 * 16) + ((tx + bx * 8) % 128) * 256 + batch_id * 32768];
    temp_6 = inputs[(ty + 6 * 16) + ((tx + bx * 8) % 128) * 256 + batch_id * 32768];
    temp_7 = inputs[(ty + 7 * 16) + ((tx + bx * 8) % 128) * 256 + batch_id * 32768];
    temp_8 = inputs[(ty + 8 * 16) + ((tx + bx * 8) % 128) * 256 + batch_id * 32768];
    temp_9 = inputs[(ty + 9 * 16) + ((tx + bx * 8) % 128) * 256 + batch_id * 32768];
    temp_10 = inputs[(ty + 10 * 16) + ((tx + bx * 8) % 128) * 256 + batch_id * 32768];
    temp_11 = inputs[(ty + 11 * 16) + ((tx + bx * 8) % 128) * 256 + batch_id * 32768];
    temp_12 = inputs[(ty + 12 * 16) + ((tx + bx * 8) % 128) * 256 + batch_id * 32768];
    temp_13 = inputs[(ty + 13 * 16) + ((tx + bx * 8) % 128) * 256 + batch_id * 32768];
    temp_14 = inputs[(ty + 14 * 16) + ((tx + bx * 8) % 128) * 256 + batch_id * 32768];
    temp_15 = inputs[(ty + 15 * 16) + ((tx + bx * 8) % 128) * 256 + batch_id * 32768];
...
temp_15 = tmp;
    ...
    temp_1 = sdata[tx + 8 * (16 + ty)];
    __id[1] = ty + 16;
    
    temp_2 = sdata[tx + 8 * (32 + ty)];
    __id[2] = ty + 32;
    
    temp_3 = sdata[tx + 8 * (48 + ty)];
    __id[3] = ty + 48;
    
    temp_4 = sdata[tx + 8 * (64 + ty)];
    __id[4] = ty + 64;
    
    temp_5 = sdata[tx + 8 * (80 + ty)];
    __id[5] = ty + 80;
    
    temp_6 = sdata[tx + 8 * (96 + ty)];
    __id[6] = ty + 96;
    
    temp_7 = sdata[tx + 8 * (112 + ty)];
    __id[7] = ty + 112;
    
    temp_8 = sdata[tx + 8 * (128 + ty)];
    __id[8] = ty + 128;
    
    temp_9 = sdata[tx + 8 * (144 + ty)];
    __id[9] = ty + 144;
    
    temp_10 = sdata[tx + 8 * (160 + ty)];
    __id[10] = ty + 160;
    
    temp_11 = sdata[tx + 8 * (176 + ty)];
    __id[11] = ty + 176;
    
    temp_12 = sdata[tx + 8 * (192 + ty)];
    __id[12] = ty + 192;
    
    temp_13 = sdata[tx + 8 * (208 + ty)];
    __id[13] = ty + 208;
    
    temp_14 = sdata[tx + 8 * (224 + ty)];
    __id[14] = ty + 224;
    
    temp_15 = sdata[tx + 8 * (240 + ty)];
    __id[15] = ty + 240;
    ...
    outputs[((tx + bx * 8) % 128) + batch_id  * 32768 + 128 * __id[0]] = temp_0;
    
    outputs[((tx + bx * 8) % 128) + batch_id  * 32768 + 128 * __id[8]] = temp_8;
    
    outputs[((tx + bx * 8) % 128) + batch_id  * 32768 + 128 * __id[4]] = temp_4;
    
    outputs[((tx + bx * 8) % 128) + batch_id  * 32768 + 128 * __id[12]] = temp_12;
    
    outputs[((tx + bx * 8) % 128) + batch_id  * 32768 + 128 * __id[2]] = temp_2;
    
    outputs[((tx + bx * 8) % 128) + batch_id  * 32768 + 128 * __id[10]] = temp_10;
    
    outputs[((tx + bx * 8) % 128) + batch_id  * 32768 + 128 * __id[6]] = temp_6;
    
    outputs[((tx + bx * 8) % 128) + batch_id  * 32768 + 128 * __id[14]] = temp_14;
    
    outputs[((tx + bx * 8) % 128) + batch_id  * 32768 + 128 * __id[1]] = temp_1;
    
    outputs[((tx + bx * 8) % 128) + batch_id  * 32768 + 128 * __id[9]] = temp_9;
    
    outputs[((tx + bx * 8) % 128) + batch_id  * 32768 + 128 * __id[5]] = temp_5;
    
    outputs[((tx + bx * 8) % 128) + batch_id  * 32768 + 128 * __id[13]] = temp_13;
    
    outputs[((tx + bx * 8) % 128) + batch_id  * 32768 + 128 * __id[3]] = temp_3;
    
    outputs[((tx + bx * 8) % 128) + batch_id  * 32768 + 128 * __id[11]] = temp_11;
    
    outputs[((tx + bx * 8) % 128) + batch_id  * 32768 + 128 * __id[7]] = temp_7;
    
    outputs[((tx + bx * 8) % 128) + batch_id  * 32768 + 128 * __id[15]] = temp_15;
