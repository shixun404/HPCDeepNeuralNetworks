void baseline_phased_fft(int num_tests, int M, int N, int K,  cublasHandle_t handle, float* dA, float* dB, float* dC, float* dE, float* dRes, float* dcheck_C_row, float* dcheck_C_col, float* dcheck_A_col_mul_B, float* dcheck_B_row_mul_A, float* dcheck_A_col, float* dcheck_B_row,  float alpha, float beta, float negative_1){
