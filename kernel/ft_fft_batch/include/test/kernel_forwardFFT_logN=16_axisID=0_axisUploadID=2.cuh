...
extern "C" __global__ void __launch_bounds__(1024) VkFFT_main_logN16_2 (float2* inputs, float2* outputs) {
...
float2* sdata = (float2*)shared;
	...
	float angle=0;
		disableThreads = (threadIdx.y * 512 + (((blockIdx.x) % 1) * 512 + ((blockIdx.x) / 1) * 8192) < 65536) ? 1 : 0;
		if(disableThreads>0) {
	...
	sdata[sdataID] = temp_7;
		...
		if (((threadIdx.x + 64 * threadIdx.y) % 16 + ((blockIdx.x) / 1)*16 < 128)){
		combinedID = (threadIdx.x + 64 * threadIdx.y) + 0;
		inoutID = combinedID % 16 + ((blockIdx.x) / 1)*16 + ((combinedID/16) * 128)+ ((blockIdx.x) % 1) * 128;
			inoutID = (inoutID) * 1;
