...
extern "C" __global__ void __launch_bounds__(256) VkFFT_main_logN24_3 (float2* inputs, float2* outputs) {
...
float2* sdata = (float2*)shared;
	...
	float angle=0;
		unsigned int numActiveThreads = ((blockIdx.x/256)==16) ? 0 : 256;
		disableThreads = (threadIdx.x * 65536 + (((blockIdx.x) % 256) * 256 + ((blockIdx.x) / 256) * 1048576) < 16777216) ? 1 : 0;
		if((threadIdx.x+16*threadIdx.y)< numActiveThreads) {
		combinedID = (threadIdx.x + 16 * threadIdx.y) + 0*numActiveThreads;
		inoutID = (combinedID % 256) + (combinedID / 256) * 65536 + (((blockIdx.x) % 256) * 256 + ((blockIdx.x) / 256) * 1048576);
			inoutID = (inoutID);
		...
		inoutID = (combinedID % 256) + (combinedID / 256) * 65536 + (((blockIdx.x) % 256) * 256 + ((blockIdx.x) / 256) * 1048576);
			inoutID = (inoutID);
		sdata[(combinedID / 256) + sharedStride*(combinedID % 256)] = inputs[inoutID];
		combinedID = (threadIdx.x + 16 * threadIdx.y) + 8*numActiveThreads;
		inoutID = (combinedID % 256) + (combinedID / 256) * 65536 + (((blockIdx.x) % 256) * 256 + ((blockIdx.x) / 256) * 1048576);
			inoutID = (inoutID);
		sdata[(combinedID / 256) + sharedStride*(combinedID % 256)] = inputs[inoutID];
		combinedID = (threadIdx.x + 16 * threadIdx.y) + 9*numActiveThreads;
		inoutID = (combinedID % 256) + (combinedID / 256) * 65536 + (((blockIdx.x) % 256) * 256 + ((blockIdx.x) / 256) * 1048576);
			inoutID = (inoutID);
		sdata[(combinedID / 256) + sharedStride*(combinedID % 256)] = inputs[inoutID];
		combinedID = (threadIdx.x + 16 * threadIdx.y) + 10*numActiveThreads;
		inoutID = (combinedID % 256) + (combinedID / 256) * 65536 + (((blockIdx.x) % 256) * 256 + ((blockIdx.x) / 256) * 1048576);
			inoutID = (inoutID);
		sdata[(combinedID / 256) + sharedStride*(combinedID % 256)] = inputs[inoutID];
		combinedID = (threadIdx.x + 16 * threadIdx.y) + 11*numActiveThreads;
		inoutID = (combinedID % 256) + (combinedID / 256) * 65536 + (((blockIdx.x) % 256) * 256 + ((blockIdx.x) / 256) * 1048576);
			inoutID = (inoutID);
		sdata[(combinedID / 256) + sharedStride*(combinedID % 256)] = inputs[inoutID];
		combinedID = (threadIdx.x + 16 * threadIdx.y) + 12*numActiveThreads;
		inoutID = (combinedID % 256) + (combinedID / 256) * 65536 + (((blockIdx.x) % 256) * 256 + ((blockIdx.x) / 256) * 1048576);
			inoutID = (inoutID);
		sdata[(combinedID / 256) + sharedStride*(combinedID % 256)] = inputs[inoutID];
		combinedID = (threadIdx.x + 16 * threadIdx.y) + 13*numActiveThreads;
		inoutID = (combinedID % 256) + (combinedID / 256) * 65536 + (((blockIdx.x) % 256) * 256 + ((blockIdx.x) / 256) * 1048576);
			inoutID = (inoutID);
		sdata[(combinedID / 256) + sharedStride*(combinedID % 256)] = inputs[inoutID];
		combinedID = (threadIdx.x + 16 * threadIdx.y) + 14*numActiveThreads;
		inoutID = (combinedID % 256) + (combinedID / 256) * 65536 + (((blockIdx.x) % 256) * 256 + ((blockIdx.x) / 256) * 1048576);
			inoutID = (inoutID);
		sdata[(combinedID / 256) + sharedStride*(combinedID % 256)] = inputs[inoutID];
		combinedID = (threadIdx.x + 16 * threadIdx.y) + 15*numActiveThreads;
		inoutID = (combinedID % 256) + (combinedID / 256) * 65536 + (((blockIdx.x) % 256) * 256 + ((blockIdx.x) / 256) * 1048576);
			inoutID = (inoutID);
		sdata[(combinedID / 256) + sharedStride*(combinedID % 256)] = inputs[inoutID];
	}
	__syncthreads();
		...
		temp_10 = sdata[sharedStride*(threadIdx.y+160)+threadIdx.x];
		temp_11 = sdata[sharedStride*(threadIdx.y+176)+threadIdx.x];
		temp_12 = sdata[sharedStride*(threadIdx.y+192)+threadIdx.x];
		temp_13 = sdata[sharedStride*(threadIdx.y+208)+threadIdx.x];
		temp_14 = sdata[sharedStride*(threadIdx.y+224)+threadIdx.x];
	...
	sdata[sdataID] = temp_15;
		...
		if (((threadIdx.x + 16 * threadIdx.y) % 16 + ((blockIdx.x) / 256)*16 < 256)){
		...
		inoutID = combinedID % 16 + ((blockIdx.x) / 256)*16 + ((combinedID/16) * 65536)+ ((blockIdx.x) % 256) * 256;
			inoutID = (inoutID) * 1;
			outputs[inoutID] = sdata[(combinedID % blockDim.x)+(combinedID/blockDim.x)*sharedStride];
		combinedID = (threadIdx.x + 16 * threadIdx.y) + 1280;
		inoutID = combinedID % 16 + ((blockIdx.x) / 256)*16 + ((combinedID/16) * 65536)+ ((blockIdx.x) % 256) * 256;
			inoutID = (inoutID) * 1;
			outputs[inoutID] = sdata[(combinedID % blockDim.x)+(combinedID/blockDim.x)*sharedStride];
		...
		inoutID = combinedID % 16 + ((blockIdx.x) / 256)*16 + ((combinedID/16) * 65536)+ ((blockIdx.x) % 256) * 256;
			inoutID = (inoutID) * 1;
			outputs[inoutID] = sdata[(combinedID % blockDim.x)+(combinedID/blockDim.x)*sharedStride];
		combinedID = (threadIdx.x + 16 * threadIdx.y) + 1792;
		inoutID = combinedID % 16 + ((blockIdx.x) / 256)*16 + ((combinedID/16) * 65536)+ ((blockIdx.x) % 256) * 256;
			inoutID = (inoutID) * 1;
			outputs[inoutID] = sdata[(combinedID % blockDim.x)+(combinedID/blockDim.x)*sharedStride];
		...
		inoutID = combinedID % 16 + ((blockIdx.x) / 256)*16 + ((combinedID/16) * 65536)+ ((blockIdx.x) % 256) * 256;
			inoutID = (inoutID) * 1;
			outputs[inoutID] = sdata[(combinedID % blockDim.x)+(combinedID/blockDim.x)*sharedStride];
		combinedID = (threadIdx.x + 16 * threadIdx.y) + 2304;
		inoutID = combinedID % 16 + ((blockIdx.x) / 256)*16 + ((combinedID/16) * 65536)+ ((blockIdx.x) % 256) * 256;
			inoutID = (inoutID) * 1;
			outputs[inoutID] = sdata[(combinedID % blockDim.x)+(combinedID/blockDim.x)*sharedStride];
		...
		inoutID = combinedID % 16 + ((blockIdx.x) / 256)*16 + ((combinedID/16) * 65536)+ ((blockIdx.x) % 256) * 256;
			inoutID = (inoutID) * 1;
			outputs[inoutID] = sdata[(combinedID % blockDim.x)+(combinedID/blockDim.x)*sharedStride];
		combinedID = (threadIdx.x + 16 * threadIdx.y) + 2816;
		inoutID = combinedID % 16 + ((blockIdx.x) / 256)*16 + ((combinedID/16) * 65536)+ ((blockIdx.x) % 256) * 256;
			inoutID = (inoutID) * 1;
			outputs[inoutID] = sdata[(combinedID % blockDim.x)+(combinedID/blockDim.x)*sharedStride];
		...
		inoutID = combinedID % 16 + ((blockIdx.x) / 256)*16 + ((combinedID/16) * 65536)+ ((blockIdx.x) % 256) * 256;
			inoutID = (inoutID) * 1;
			outputs[inoutID] = sdata[(combinedID % blockDim.x)+(combinedID/blockDim.x)*sharedStride];
		combinedID = (threadIdx.x + 16 * threadIdx.y) + 3328;
		inoutID = combinedID % 16 + ((blockIdx.x) / 256)*16 + ((combinedID/16) * 65536)+ ((blockIdx.x) % 256) * 256;
			inoutID = (inoutID) * 1;
			outputs[inoutID] = sdata[(combinedID % blockDim.x)+(combinedID/blockDim.x)*sharedStride];
		...
		inoutID = combinedID % 16 + ((blockIdx.x) / 256)*16 + ((combinedID/16) * 65536)+ ((blockIdx.x) % 256) * 256;
			inoutID = (inoutID) * 1;
			outputs[inoutID] = sdata[(combinedID % blockDim.x)+(combinedID/blockDim.x)*sharedStride];
		combinedID = (threadIdx.x + 16 * threadIdx.y) + 3840;
		inoutID = combinedID % 16 + ((blockIdx.x) / 256)*16 + ((combinedID/16) * 65536)+ ((blockIdx.x) % 256) * 256;
			inoutID = (inoutID) * 1;
			outputs[inoutID] = sdata[(combinedID % blockDim.x)+(combinedID/blockDim.x)*sharedStride];
	}
