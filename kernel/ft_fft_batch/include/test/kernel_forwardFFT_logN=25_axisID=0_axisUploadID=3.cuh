...
extern "C" __global__ void __launch_bounds__(1024) VkFFT_main_logN25_3 (float2* inputs, float2* outputs) {
...
float2* sdata = (float2*)shared;
	...
	float angle=0;
		disableThreads = (threadIdx.y * 131072 + (((blockIdx.x) % 256) * 512 + ((blockIdx.x) / 256) * 2097152) < 33554432) ? 1 : 0;
		if(disableThreads>0) {
	...
	sdata[sdataID] = temp_7;
		...
		if (((threadIdx.x + 64 * threadIdx.y) % 16 + ((blockIdx.x) / 256)*16 < 256)){
		combinedID = (threadIdx.x + 64 * threadIdx.y) + 0;
		inoutID = combinedID % 16 + ((blockIdx.x) / 256)*16 + ((combinedID/16) * 65536)+ ((blockIdx.x) % 256) * 256;
			inoutID = (inoutID) * 1;
