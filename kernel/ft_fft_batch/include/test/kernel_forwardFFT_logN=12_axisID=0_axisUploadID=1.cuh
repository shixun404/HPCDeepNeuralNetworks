...
extern "C" __global__ void __launch_bounds__(256) VkFFT_main_logN12 (float2* inputs, float2* outputs) {
unsigned int sharedStride = 4112;
float2* sdata = (float2*)shared;
	...
	float angle=0;
		...
		inoutID = (combinedID % 4096) + (combinedID / 4096) * 4096;
			inoutID = (inoutID);
		...
		combinedID = threadIdx.x + 2304;
		inoutID = (combinedID % 4096) + (combinedID / 4096) * 4096;
			inoutID = (inoutID);
		temp_9 = inputs[inoutID];
		combinedID = threadIdx.x + 2560;
		inoutID = (combinedID % 4096) + (combinedID / 4096) * 4096;
			inoutID = (inoutID);
		temp_10 = inputs[inoutID];
		combinedID = threadIdx.x + 2816;
		inoutID = (combinedID % 4096) + (combinedID / 4096) * 4096;
			inoutID = (inoutID);
		...
		combinedID = threadIdx.x + 3328;
		inoutID = (combinedID % 4096) + (combinedID / 4096) * 4096;
			inoutID = (inoutID);
		temp_13 = inputs[inoutID];
		combinedID = threadIdx.x + 3584;
		inoutID = (combinedID % 4096) + (combinedID / 4096) * 4096;
			inoutID = (inoutID);
		temp_14 = inputs[inoutID];
		combinedID = threadIdx.x + 3840;
		inoutID = (combinedID % 4096) + (combinedID / 4096) * 4096;
			inoutID = (inoutID);
	...
	sdataID = inoutID + 0;
	sharedStride = 4352;	sdataID = (sdataID / 16) * 17 + sdataID % 16;	sdata[sdataID] = temp_0;
	sdataID = inoutID + 1;
