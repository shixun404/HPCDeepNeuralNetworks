...
# '''
    ...
    __global__ void __launch_bounds__({num_thread}) fft_radix{radix}_logN{exponent}''' + '''(float2* inputs, float2* outputs, float2* r_2) {
    '''
        
        ft_fft += f'''
    // r_1 = constData;
    // float2* r_1 = r_2 + {2 ** exponent} * (blockIdx.x % 1);
    float2* r_1 = r_2;
    float2 r[3];
    r[0].x = 1.0f;
    r[0].y = 0.0f;
    ...
    '''
        ...
        if 2 * N // num_thread >= 1 and (2 * N // num_thread <= 4):
            ft_fft += f'''
        #if FT==2
    ...
    '''
        ...
        '''
                for i in range(signal_per_thread):    
                    ft_fft += f'''
    MY_ANGLE2COMPLEX((float)(-M_PI * 2 * ((ty) / {int(N // N__)}) * {i}) / (float)({N__}), tmp_angle);
    MY_MUL(temp_{order[signal_per_thread - offset + i]}, tmp_angle, tmp);
    temp_{order[signal_per_thread - offset + i]} = tmp;
    '''
                for i in range(signal_per_thread):
